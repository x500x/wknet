        ...
        AsyncOp** operation) noexcept
    {
        ...
        if (options != nullptr) {
            ...
            apiOptions.CompletionCallback =
                reinterpret_cast<::KernelHttp::engine::KhAsyncCompletionCallback>(options->OnComplete);
        ...
        status = ::KernelHttp::engine::KhHttpSendAsync(session->Engine, apiRequest, apiOptionsPtr, &apiOp);
