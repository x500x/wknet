...
namespace
{
    ...
    UCHAR* CopyBytes(const UCHAR* data, SIZE_T length) noexcept
    {
        ...
        UCHAR* copy = ::wknet::AllocateNonPagedArray<UCHAR>(length);
        if (copy == nullptr) {
            return nullptr;
        }
    ...
    NTSTATUS AllocateBody(Body** body) noexcept
    {
        ...
        auto* created = ::wknet::AllocateNonPagedObject<Body>();
        if (created == nullptr) {
...
NTSTATUS BodyCreateForm(const NameValuePair* pairs, SIZE_T pairCount, Body** body) noexcept
{
    ...
    created->FormPairs = ::wknet::AllocateNonPagedArray<NameValuePair>(pairCount);
...
NTSTATUS BodyCreateMultipart(const MultipartPart* parts, SIZE_T partCount, Body** body) noexcept
{
    ...
    created->MultipartParts = ::wknet::AllocateNonPagedArray<MultipartPart>(partCount);
...
void BodyRelease(Body* body) noexcept
{
    ...
    if (body->OwnsData) {
        ::wknet::FreeNonPagedArray(const_cast<UCHAR*>(body->Data));
    }
    ::wknet::FreeNonPagedArray(body->ContentType);
