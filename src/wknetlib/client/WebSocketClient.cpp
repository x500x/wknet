...
namespace client
{
    namespace
    {
        ...
        {
            const http1::HttpText rightText = http1::MakeText(right);
            ...
            _Inout_ SIZE_T* length) noexcept
        {
            ...
            const http1::HttpText text = http1::MakeText(literal);
        ...
        NTSTATUS AppendWindowBits(
            ...
            _Inout_ SIZE_T* length) noexcept
        {
            if (value < ws::WebSocketDeflateMinWindowBits ||
                value > ws::WebSocketDeflateMaxWindowBits ||
                destination == nullptr ||
                length == nullptr) {
                return STATUS_INVALID_PARAMETER;
            }
        ...
        NTSTATUS BuildPerMessageDeflateOffer(
            const ws::PerMessageDeflateOptions& options,
            _Out_writes_bytes_(destinationCapacity) char* destination,
            SIZE_T destinationCapacity,
            _Out_ SIZE_T* length) noexcept
        {
            ...
            if (!ws::IsValidPerMessageDeflateOptions(options) || !options.Enable) {
                return STATUS_INVALID_PARAMETER;
            }
        ...
        _Must_inspect_result_
        bool IsForbiddenHttp2OpeningHeader(http1::HttpText name) noexcept
        {
            return http1::TextEqualsIgnoreCase(name, http1::MakeText("connection")) ||
                ...
                http1::TextEqualsIgnoreCase(name, http1::MakeText("sec-websocket-key")) ||
                http1::TextEqualsIgnoreCase(name, http1::MakeText("sec-websocket-version")) ||
                http1::TextEqualsIgnoreCase(name, http1::MakeText("sec-websocket-protocol")) ||
                http1::TextEqualsIgnoreCase(name, http1::MakeText("sec-websocket-extensions"));
        }
            ...
            _Out_ SIZE_T* bytesWritten) noexcept
        {
            ...
            HeapArray<http1::HttpHeader> headers(headerCount);
            if (!headers.IsValid()) {
                return STATUS_INSUFFICIENT_RESOURCES;
            }

            SIZE_T nextHeader = 0;
            headers[nextHeader++] = { http1::MakeText("Upgrade"), http1::MakeText("websocket") };
            headers[nextHeader++] = { http1::MakeText("Sec-WebSocket-Key"), { clientKey, clientKeyLength } };
            headers[nextHeader++] = { http1::MakeText("Sec-WebSocket-Version"), http1::MakeText("13") };
            if (options.Subprotocol != nullptr && options.SubprotocolLength != 0) {
                headers[nextHeader++] = {
                    http1::MakeText("Sec-WebSocket-Protocol"),
                    { options.Subprotocol, options.SubprotocolLength }
                };
            }
            if (options.PerMessageDeflate.Enable) {
                headers[nextHeader++] = {
                    http1::MakeText("Sec-WebSocket-Extensions"),
            ...
            request.Connection = http1::HttpConnectionDirective::Upgrade;
            ...
            _Out_ SIZE_T* headerCount) noexcept
        {
            ...
            SIZE_T nextHeader = 0;
            headers[nextHeader++] = { http1::MakeText(":method"), http1::MakeText("CONNECT") };
            headers[nextHeader++] = { http1::MakeText(":scheme"), http1::MakeText("https") };
            headers[nextHeader++] = { http1::MakeText(":path"), { options.Path, options.PathLength } };
            headers[nextHeader++] = { http1::MakeText(":authority"), { authority, authorityLength } };
            headers[nextHeader++] = { http1::MakeText(":protocol"), http1::MakeText("websocket") };
            headers[nextHeader++] = { http1::MakeText("user-agent"), http1::MakeText("wknet/0.1") };
            if (options.Subprotocol != nullptr && options.SubprotocolLength != 0) {
                headers[nextHeader++] = {
                    http1::MakeText("sec-websocket-protocol"),
                    { options.Subprotocol, options.SubprotocolLength }
            ...
            for (SIZE_T index = 0; index < options.ExtraHeaderCount; ++index) {
                const http1::HttpHeader& source = options.ExtraHeaders[index];
        ...
        NTSTATUS ValidateHttp2WebSocketResponse(
            ...
            _Out_ http1::HttpText* selectedSubprotocol,
            _In_opt_ const ws::PerMessageDeflateOptions* perMessageDeflate,
            _Out_opt_ ws::PerMessageDeflateNegotiation* negotiatedDeflate) noexcept
        {
            ...
            NTSTATUS status = ws::WebSocketCodec::ValidatePerMessageDeflateExtensions(
                headers,
                headerCount,
                http1::MakeText("sec-websocket-extensions"),
                perMessageDeflate,
        ...
        NTSTATUS TryDecodeCompleteFrame(
            ...
            SIZE_T dataLength,
            _Out_ ws::WebSocketFrameHeader* header,
            ...
            _Out_ bool* complete) noexcept
        {
            ...
            NTSTATUS status = ws::WebSocketCodec::DecodeFrameHeader(data, dataLength, header);
        ...
        {
            return frameBufferLength > ws::WebSocketFrameHeaderMaxLength ?
                frameBufferLength - ws::WebSocketFrameHeaderMaxLength :
                0;
        }
        ...
        USHORT* statusCode) noexcept
    {
        ...
        HeapArray<char> clientKey(ws::WebSocketClientKeyBase64Length);
        if (!clientKey.IsValid()) {
            return STATUS_INSUFFICIENT_RESOURCES;
        }

        SIZE_T clientKeyLength = 0;
        status = ws::WebSocketCodec::GenerateClientKey(
            clientKey.Get(),
            clientKey.Count(),
            &clientKeyLength);
        if (!NT_SUCCESS(status)) {
        ...
        bool* tls12ConfirmationCandidate) noexcept
    {
        ...
        if (useTls_) {
            ...
            if (WebSocketModeAllowsHttp2(options) &&
                ...
                HeapArray<http1::HttpHeader> requestHeaders(
                    WebSocketHttp2BaseHeaderCount +
                    subprotocolHeaderCount +
                    extensionHeaderCount +
                    options.ExtraHeaderCount);
                ...
                http1::HttpText selectedSubprotocol = {};
        ...
        for (;;) {
            ...
            if (status == STATUS_NOT_SUPPORTED &&
                ...
                HeapArray<http1::HttpHeader> combinedHeaders(combinedHeaderCount);
        ...
        bool finalFragment) noexcept
    {
        ...
        return SendFrame(
            ws::WebSocketOpcode::Continuation,
            message,
            messageLength,
            buffers,
            finalFragment);
        ...
        const WebSocketIoBuffers& buffers) noexcept
    {
        ...
        if (opcode != ws::WebSocketOpcode::Ping &&
            opcode != ws::WebSocketOpcode::Pong) {
            return STATUS_INVALID_PARAMETER;
        }
        ...
        bool finalFragment) noexcept
    {
        ...
        const bool continuation = opcode == ws::WebSocketOpcode::Continuation;
        if (continuation != sendFragmentOpen_) {
            return STATUS_INVALID_DEVICE_STATE;
        }
        if (!continuation &&
            opcode != ws::WebSocketOpcode::Text &&
            opcode != ws::WebSocketOpcode::Binary) {
            return STATUS_INVALID_PARAMETER;
        }
        ...
        const bool textFragment =
            opcode == ws::WebSocketOpcode::Text ||
            (continuation && sendFragmentOpcode_ == ws::WebSocketOpcode::Text);
        if (textFragment) {
            if (opcode == ws::WebSocketOpcode::Text) {
                nextUtf8CodePoint = 0;
                nextUtf8Remaining = 0;
                nextUtf8Expected = 0;
            }
        ...
        while (!sentAnyFrame || offset < messageLength) {
            ...
            const ws::WebSocketOpcode frameOpcode =
                sentAnyFrame ? ws::WebSocketOpcode::Continuation : opcode;
        ...
        bool* finalFragment) noexcept
    {
        ...
        if (outputCapacity > static_cast<SIZE_T>(-1) - ws::WebSocketFrameHeaderMaxLength) {
            return STATUS_BUFFER_TOO_SMALL;
        }

        const SIZE_T minimumPayloadCapacity =
            outputCapacity < ws::WebSocketMaxControlPayloadLength ?
            ws::WebSocketMaxControlPayloadLength :
            outputCapacity;
        if (minimumPayloadCapacity > static_cast<SIZE_T>(-1) - ws::WebSocketFrameHeaderMaxLength) {
            return STATUS_BUFFER_TOO_SMALL;
        }

        SIZE_T receiveFrameCapacity = minimumPayloadCapacity + ws::WebSocketFrameHeaderMaxLength;
        ...
        ws::WebSocketFrameHeader& header = *receiveFrameHeader_.Get();
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if (headerDecoded) {
                    ...
                    const bool dataFrame =
                        header.Opcode == ws::WebSocketOpcode::Text ||
                        header.Opcode == ws::WebSocketOpcode::Binary;
                    const bool controlFrame =
                        header.Opcode == ws::WebSocketOpcode::Ping ||
                        header.Opcode == ws::WebSocketOpcode::Pong ||
                        header.Opcode == ws::WebSocketOpcode::Close;
                    if (header.Rsv1 &&
                        (controlFrame ||
                            header.Opcode == ws::WebSocketOpcode::Continuation ||
                    ...
                    if (header.Opcode == ws::WebSocketOpcode::Continuation && !receiveFragmentOpen_) {
                        return FailConnectionWithClose(
                            WebSocketCloseProtocolError,
                            buffers,
                            STATUS_INVALID_NETWORK_RESPONSE);
                    }
                    if (dataFrame || header.Opcode == ws::WebSocketOpcode::Continuation) {
                ...
                if (!autoReplyPing) {
                    if (receiveFragmentOpen_) {
                        *bytesReceived = 0;
                        ...
                        status = ws::WebSocketCodec::DecodeFramePayload(
                            header,
                            bufferedFrame_,
                            frameLength,
                            output,
            ...
            if (header.Opcode == ws::WebSocketOpcode::Close) {
            ...
            const bool textFragment =
                receiveFragmentOpcode_ == ws::WebSocketOpcode::Text;
            ...
            if (receiveCompressedMessage_) {
                ...
                if (receiveFragmentOpcode_ == ws::WebSocketOpcode::Text) {
    ...
    NTSTATUS WebSocketClient::WaitForPeerClose(const WebSocketIoBuffers& buffers) noexcept
    {
        ...
        const SIZE_T closeFrameCapacity =
            ws::WebSocketFrameHeaderMaxLength + ws::WebSocketMaxControlPayloadLength;
        ...
        WebSocketEchoResult& result) noexcept
    {
        ...
        for (SIZE_T frameIndex = 0; frameIndex < MaxFramesBeforeEcho; ++frameIndex) {
            ...
            if (result.Opcode == ws::WebSocketOpcode::Text &&
                PayloadEquals(message, messageLength, buffers.PayloadBuffer, result.BytesReceived)) {
                return STATUS_SUCCESS;
            }

            if (result.Opcode == ws::WebSocketOpcode::Close) {
                return STATUS_CONNECTION_DISCONNECTED;
            }
        }
    ...
    void WebSocketClient::ResetReceiveFragment() noexcept
    {
        ...
        receiveCompressedMessage_ = false;
        receiveFragmentOpcode_ = ws::WebSocketOpcode::Continuation;
    ...
    void WebSocketClient::ResetSendFragment() noexcept
    {
        ...
        sendCompressedMessage_ = false;
        sendFragmentOpcode_ = ws::WebSocketOpcode::Continuation;
        ...
        bool rsv1) noexcept
    {
        if (h2Connection_ != nullptr && h2Transport_ != nullptr && h2StreamId_ != 0) {
            ...
            NTSTATUS status = ws::WebSocketCodec::EncodeClientFrameForHttp2(
                opcode,
                finalFragment,
                rsv1,
                payload,
        ...
        HeapArray<UCHAR> maskingKey(ws::WebSocketMaskingKeyLength);
        ...
        NTSTATUS status = crypto::CngProvider::GenerateRandom(
            maskingKey.Get(),
            ws::WebSocketMaskingKeyLength);
        if (NT_SUCCESS(status)) {
            SIZE_T frameLength = 0;
            status = ws::WebSocketCodec::EncodeClientFrame(
                opcode,
                finalFragment,
                rsv1,
                payload,
        ...
        bool finalFragment) noexcept
    {
        ...
        const SIZE_T required = ws::WebSocketDeflateContext::MaxStoredDeflateBytes(
            payloadLength,
            finalFragment);
        ...
        NTSTATUS status = ws::WebSocketDeflateContext::DeflateMessage(
            payload,
            payloadLength,
            finalFragment,
        ...
        http1::HttpResponse& response) noexcept
    {
        ...
        for (;;) {
            ...
            if (status == STATUS_SUCCESS) {
                ...
                http1::HttpText selectedSubprotocol = {};
                status = ws::WebSocketCodec::ValidateServerHandshake(
                    response,
                    clientKey,
                    clientKeyLength,
