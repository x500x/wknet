...
namespace client
{
    namespace
    {
        ...
        bool TextEqualsIgnoreCase(http1::HttpText text, const char* literal) noexcept
        {
        ...
        bool LowercaseHeaderName(http1::HttpText name, char* output, SIZE_T capacity, http1::HttpText& lowered) noexcept
        {
        ...
        bool WriteDecimal(SIZE_T value, char* output, SIZE_T capacity, http1::HttpText& text) noexcept
        {
        ...
        bool GetAuthority(const Http2RequestOptions& options, http1::HttpText& authority) noexcept
        {
        ...
        NTSTATUS BuildHttp2SettingsHeader(char* output, SIZE_T outputCapacity, http1::HttpText& value) noexcept
        {
        ...
        NTSTATUS AppendText(char* output, SIZE_T capacity, SIZE_T& offset, http1::HttpText text) noexcept
        {
        ...
        bool HeaderContainsToken(const char* data, SIZE_T length, const char* headerName, const char* token) noexcept
        {
            ...
            for (SIZE_T i = 0; i + nameLength < length; ++i) {
                http1::HttpText name = { data + i, nameLength };
        ...
        SIZE_T* headerCount) noexcept
    {
        ...
        if (usesExtendedConnect && options.Method != http1::HttpMethod::Connect) {
            return STATUS_INVALID_PARAMETER;
        }
        ...
        requestHeaders[headerIdx].Value = IsTlsMode(options.TransportMode) ? http1::HttpText{ "https", 5 } : http1::HttpText{ "http", 4 };
        ++headerIdx;
