...
QuicAttemptValidation::~QuicAttemptValidation() noexcept
{
    Reset();
}
...
void QuicAttemptValidation::Reset() noexcept
{
    ...
    if (retryToken_.IsValid())
        RtlSecureZeroMemory(retryToken_.Get(), retryToken_.Count());
    retryToken_.Reset();
    originalVersion_ = 0;
...
NTSTATUS QuicAttemptValidation::Initialize(ULONG originalVersion, QuicBufferView odcid,
                                           QuicBufferView clientScid) noexcept
{
    Reset();
    if (originalVersion == 0 || odcid.Data == nullptr || odcid.Length == 0 ||
        odcid.Length > QuicMaximumConnectionIdLength || (clientScid.Data == nullptr && clientScid.Length != 0) ||
        clientScid.Length > QuicMaximumConnectionIdLength)
...
NTSTATUS QuicAttemptValidation::ValidateVersionNegotiation(const QuicPacketHeader &packet,
                                                           const ULONG *supportedVersions, SIZE_T supportedVersionCount,
                                                           ULONG *selectedVersion) noexcept
{
    ...
    if (!initialized_ || packet.Type != QuicPacketType::VersionNegotiation || retryAccepted_ ||
        ...
        !ViewsEqual(packet.SourceConnectionId, originalDestinationConnectionId_,
                    originalDestinationConnectionIdLength_))
    {
    ...
    if (NT_SUCCESS(status))
    {
        for (SIZE_T offset = 0; offset < packet.VersionList.Length; offset += 4)
        {
            ...
            {
                if (supportedVersions[index] == offered && offered != originalVersion_)
                    selected = offered;
            }
        }
    }
    ...
    WKNET_TRACE_CORRELATED(ComponentQuic, TraceLevel::Warning, &correlation,
                           ...
                           static_cast<ULONG>(status), packet.DestinationConnectionId.Length,
                           packet.SourceConnectionId.Length);
    return status;
}

NTSTATUS QuicAttemptValidation::ValidateRetry(const QuicPacketHeader &packet, const UCHAR *packetBytes,
                                              SIZE_T packetLength) noexcept
{
    NTSTATUS status = STATUS_SUCCESS;
    if (!initialized_ || packet.Type != QuicPacketType::Retry || retryAccepted_ || versionNegotiationAccepted_ ||
        packet.Version != originalVersion_ ||
        !ViewsEqual(packet.DestinationConnectionId, clientSourceConnectionId_, clientSourceConnectionIdLength_) ||
    ...
    if (NT_SUCCESS(status))
        status = QuicValidateRetryIntegrityTag(
    ...
    {
        WKNET_TRACE_CORRELATED(ComponentQuic, TraceLevel::Info, &correlation,
                               "quic.retry.accepted version=%u dcid_bytes=%Iu scid_bytes=%Iu", packet.Version,
    ...
    else
    {
        WKNET_TRACE_CORRELATED(ComponentQuic, TraceLevel::Warning, &correlation,
                               ...
                               static_cast<ULONG>(status), packet.Version, packet.DestinationConnectionId.Length,
                               packet.SourceConnectionId.Length);
    }
    return status;
}
