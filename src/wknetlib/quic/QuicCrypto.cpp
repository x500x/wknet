...
namespace wknet::quic
{
    namespace
    {
        ...
        {
            return suite == QuicCipherSuite::Aes256GcmSha384 ? crypto::HashAlgorithm::Sha384
                                    ...
                                    QuicPacketKeySet* keySet) noexcept
    {
        ...
        if (NT_SUCCESS(status))
            status = tls::Tls13KeySchedule::HkdfExpandLabel(hash, secret, secretLength, "quic iv", nullptr, 0,
                                                            keySet->Iv, sizeof(keySet->Iv));
        if (NT_SUCCESS(status))
            status =
                tls::Tls13KeySchedule::HkdfExpandLabel(hash, secret, secretLength, "quic hp", nullptr, 0,
    ...
    NTSTATUS QuicDeriveNextPacketKeySet(const QuicPacketKeySet& current, QuicPacketKeySet* next) noexcept
    {
        ...
        if (next == nullptr || current.SecretLength == 0 ||
            current.SecretLength != SecretLengthForSuite(current.Suite) || current.HeaderProtectionKeyLength == 0 ||
            current.HeaderProtectionKeyLength > sizeof(current.HeaderProtectionKey))
        {
            return STATUS_INVALID_PARAMETER;
        }
        ...
        if (NT_SUCCESS(status))
        {
            RtlSecureZeroMemory(next->HeaderProtectionKey, sizeof(next->HeaderProtectionKey));
            next->HeaderProtectionKeyLength = current.HeaderProtectionKeyLength;
            RtlCopyMemory(next->HeaderProtectionKey, current.HeaderProtectionKey, current.HeaderProtectionKeyLength);
        }
                                      ...
                                      QuicPacketKeySet* server) noexcept
    {
        ...
        if (NT_SUCCESS(status))
            status = QuicDerivePacketKeySet(QuicCipherSuite::Aes128GcmSha256, clientSecret.Get(), clientSecret.Count(),
                                            client);
        if (NT_SUCCESS(status))
            status = QuicDerivePacketKeySet(QuicCipherSuite::Aes128GcmSha256, serverSecret.Get(), serverSecret.Count(),
                                            server);
    ...
    NTSTATUS QuicInspectProtectedPacketHeader(const QuicPacketKeySet& keySet, const UCHAR* packet, SIZE_T packetLength,
                                              SIZE_T packetNumberOffset, ULONGLONG expectedPacketNumber,
                                              UCHAR* firstByte, ULONGLONG* packetNumber) noexcept
    {
        ...
        if (packet == nullptr || firstByte == nullptr || packetNumber == nullptr || packetNumberOffset > packetLength ||
            4 > packetLength - packetNumberOffset || 16 > packetLength - (packetNumberOffset + 4))
        {
            return STATUS_INVALID_PARAMETER;
        }

        HeapArray<UCHAR> mask(5);
        if (!mask.IsValid())
        {
            return STATUS_INSUFFICIENT_RESOURCES;
        }
        ...
        if (NT_SUCCESS(status))
        {
            *firstByte = static_cast<UCHAR>(packet[0] ^ (mask[0] & ((packet[0] & 0x80U) != 0 ? 0x0fU : 0x1fU)));
            const bool longHeader = (*firstByte & 0x80U) != 0;
            if ((longHeader && (*firstByte & 0x0cU) != 0) || (!longHeader && (*firstByte & 0x18U) != 0))
            {
                status = STATUS_INVALID_NETWORK_RESPONSE;
            }
        }
        ...
        if (NT_SUCCESS(status))
        {
            packetNumberLength = (*firstByte & 0x03U) + 1U;
            if (packetNumberLength > packetLength - packetNumberOffset)
            {
                status = STATUS_INVALID_NETWORK_RESPONSE;
            }
        }
        ULONGLONG truncatedPacketNumber = 0;
        if (NT_SUCCESS(status))
        {
            for (SIZE_T index = 0; index < packetNumberLength; ++index)
            {
                const UCHAR value = static_cast<UCHAR>(packet[packetNumberOffset + index] ^ mask[index + 1]);
                truncatedPacketNumber = (truncatedPacketNumber << 8) | value;
            }
            status = QuicReconstructPacketNumber(truncatedPacketNumber, packetNumberLength, expectedPacketNumber,
                                                 packetNumber);
        }
        RtlSecureZeroMemory(mask.Get(), mask.Count());
        return status;
    ...
    NTSTATUS QuicComputeRetryIntegrityTag(QuicBufferView odcid, const UCHAR* retryWithoutTag,
                                          SIZE_T retryWithoutTagLength, UCHAR* tag) noexcept
    {
