...
class QuicAttemptValidation final
{
  public:
    ...
    NTSTATUS Initialize(ULONG originalVersion, QuicBufferView originalDestinationConnectionId,
                        QuicBufferView clientSourceConnectionId) noexcept;

    NTSTATUS ValidateVersionNegotiation(_In_ const QuicPacketHeader &packet,
                                        _In_reads_(supportedVersionCount) const ULONG *supportedVersions,
                                        SIZE_T supportedVersionCount, _Out_opt_ ULONG *selectedVersion) noexcept;

    NTSTATUS ValidateRetry(_In_ const QuicPacketHeader &packet, _In_reads_bytes_(packetLength) const UCHAR *packetBytes,
    ...
    bool RetryAccepted() const noexcept
    {
        return retryAccepted_;
    }
    bool VersionNegotiationAccepted() const noexcept
    {
        return versionNegotiationAccepted_;
    }
    void Reset() noexcept;
