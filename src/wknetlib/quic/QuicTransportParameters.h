...
{
enum class QuicTransportParameterPeerRole : UCHAR
{
    Client,
    Server
};
...
struct QuicClientTransportParameterOptions final
{
    QuicBufferView InitialSourceConnectionId = {};
    ULONGLONG MaxIdleTimeout = 0;
    ULONGLONG InitialMaxData = 0;
...
NTSTATUS QuicParseTransportParameters(_In_reads_bytes_(dataLength) const UCHAR *data, SIZE_T dataLength,
                                      QuicTransportParameterPeerRole peerRole,
                                      _Out_ QuicTransportParameters *parameters) noexcept;

NTSTATUS QuicEncodeClientTransportParameters(_In_ QuicBufferView initialSourceConnectionId,
                                             _Out_writes_bytes_(capacity) UCHAR *output, SIZE_T capacity,
                                             _Out_ SIZE_T *bytesWritten) noexcept;

NTSTATUS QuicEncodeClientTransportParametersWithLimits(_In_ const QuicClientTransportParameterOptions &options,
                                                       _Out_writes_bytes_(capacity) UCHAR *output, SIZE_T capacity,
                                                       _Out_ SIZE_T *bytesWritten) noexcept;
} // namespace wknet::quic
