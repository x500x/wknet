...
namespace wknet::quic {
    ...
    NTSTATUS QuicDeriveInitialKeySets(
        _In_ QuicBufferView destinationConnectionId,
        _Out_ QuicPacketKeySet* client,
        _Out_ QuicPacketKeySet* server) noexcept;

    NTSTATUS QuicDerivePacketKeySet(
        QuicCipherSuite suite,
        _In_reads_bytes_(secretLength) const UCHAR* secret,
        SIZE_T secretLength,
        _Out_ QuicPacketKeySet* keySet) noexcept;

    inline NTSTATUS QuicInstallReadSecret(
    ...
    inline NTSTATUS QuicInstallWriteSecret(
        QuicCipherSuite suite, const UCHAR* secret, SIZE_T secretLength,
        QuicPacketKeySet* keySet) noexcept
    {
        return QuicDerivePacketKeySet(suite, secret, secretLength, keySet);
    ...
    NTSTATUS QuicBuildPacketNonce(
        _In_reads_bytes_(12) const UCHAR* iv,
        ULONGLONG packetNumber,
        _Out_writes_bytes_(12) UCHAR* nonce) noexcept;

    NTSTATUS QuicHeaderProtectionMask(
        _In_ const QuicPacketKeySet& keySet,
        _In_reads_bytes_(16) const UCHAR* sample,
        _Out_writes_bytes_(5) UCHAR* mask) noexcept;

    NTSTATUS QuicProtectPacket(
        _In_ const QuicPacketKeySet& keySet,
        _Inout_updates_(packetCapacity) UCHAR* packet,
        SIZE_T packetCapacity,
        SIZE_T packetNumberOffset,
        SIZE_T packetNumberLength,
        ULONGLONG packetNumber,
        SIZE_T plaintextLength,
        _Out_ SIZE_T* packetLength) noexcept;

    NTSTATUS QuicUnprotectPacket(
        _In_ const QuicPacketKeySet& keySet,
        _Inout_updates_(packetLength) UCHAR* packet,
        SIZE_T packetLength,
        SIZE_T packetNumberOffset,
        ULONGLONG expectedPacketNumber,
        _Out_ ULONGLONG* packetNumber,
        _Out_ SIZE_T* plaintextLength) noexcept;

    NTSTATUS QuicComputeRetryIntegrityTag(
        _In_ QuicBufferView originalDestinationConnectionId,
