...
SIZE_T QuicVarIntEncodedLength(ULONGLONG value) noexcept
{
    ...
    if (value <= 1073741823ULL)
        return 4;
    if (value <= QuicVarIntMaximum)
        return 8;
    return 0;
}

NTSTATUS QuicDecodeVarInt(const UCHAR *data, SIZE_T dataLength, ULONGLONG *value, SIZE_T *bytesConsumed) noexcept
{
    if (value != nullptr)
        *value = 0;
...
NTSTATUS QuicEncodeVarInt(ULONGLONG value, SIZE_T encodedLength, UCHAR *output, SIZE_T capacity,
                          SIZE_T *bytesWritten) noexcept
{
    ...
    if (encodedLength == 0)
        encodedLength = QuicVarIntEncodedLength(value);
    if (encodedLength != 1 && encodedLength != 2 && encodedLength != 4 && encodedLength != 8)
    {
        return STATUS_INVALID_PARAMETER;
    }
    const ULONGLONG maximum = encodedLength == 8 ? QuicVarIntMaximum : ((1ULL << ((encodedLength * 8) - 2)) - 1ULL);
    if (value > maximum)
        return STATUS_INTEGER_OVERFLOW;
    if (capacity < encodedLength)
        return STATUS_BUFFER_TOO_SMALL;
    ...
    const UCHAR prefix =
        encodedLength == 1 ? 0x00U : (encodedLength == 2 ? 0x40U : (encodedLength == 4 ? 0x80U : 0xc0U));
