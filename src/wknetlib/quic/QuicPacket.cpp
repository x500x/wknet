...
namespace wknet::quic {
    ...
    NTSTATUS QuicParsePacketHeader(
        ...
        QuicPacketHeader* header) noexcept
    {
        ...
        if (!header->FixedBit) return STATUS_INVALID_NETWORK_RESPONSE;
        ...
        if (!header->IsLongHeader) {
            ...
            header->DestinationConnectionId = {
                packet + 1, shortHeaderDestinationConnectionIdLength
            };
        ...
        if (packetLength < 6) return STATUS_BUFFER_TOO_SMALL;
        SIZE_T offset = 1;
        header->Version = ReadBigEndian32(packet + offset);
        offset += 4;
        const SIZE_T dcidLength = packet[offset++];
        if (dcidLength > QuicMaximumConnectionIdLength ||
            !CanAdvance(offset, dcidLength + 1, packetLength)) {
            return STATUS_INVALID_NETWORK_RESPONSE;
        }
        header->DestinationConnectionId = { packet + offset, dcidLength };
        offset += dcidLength;
        const SIZE_T scidLength = packet[offset++];
        if (scidLength > QuicMaximumConnectionIdLength ||
            !CanAdvance(offset, scidLength, packetLength)) {
            return STATUS_INVALID_NETWORK_RESPONSE;
        }
        ...
        if (header->Version != QuicVersion1) return STATUS_NOT_SUPPORTED;
        const UCHAR longType = static_cast<UCHAR>((packet[0] >> 4) & 0x03U);
        header->Type = longType == 0 ? QuicPacketType::Initial :
        ...
        if (header->Type == QuicPacketType::Retry) {
            ...
            header->RetryIntegrityTag = {
                packet + packetLength - QuicRetryIntegrityTagLength,
                QuicRetryIntegrityTagLength
            };
            header->HeaderLength = offset;
            header->PacketLength = packetLength;
            return STATUS_SUCCESS;
        ...
        header->PacketNumberLength = (packet[0] & 0x03U) + 1U;
        if (protectedLength < header->PacketNumberLength) return STATUS_INVALID_NETWORK_RESPONSE;
        ...
        SIZE_T* bytesWritten) noexcept
    {
        ...
        if (packetNumber > QuicVarIntMaximum) return STATUS_INTEGER_OVERFLOW;
        const ULONGLONG maximum = (1ULL << (packetNumberLength * 8)) - 1ULL;
        if (packetNumber > maximum) return STATUS_INTEGER_OVERFLOW;
        if (capacity < packetNumberLength) return STATUS_BUFFER_TOO_SMALL;
        for (SIZE_T index = packetNumberLength; index > 0; --index) {
            output[index - 1] = static_cast<UCHAR>(packetNumber & 0xffU);
            packetNumber >>= 8;
        }
        if (bytesWritten != nullptr) *bytesWritten = packetNumberLength;
        return STATUS_SUCCESS;
    }
    ...
    NTSTATUS QuicReconstructPacketNumber(
        ...
        ULONGLONG* packetNumber) noexcept
    {
        ...
        if (truncatedPacketNumber > mask) return STATUS_INVALID_PARAMETER;
        ULONGLONG candidate = (expectedPacketNumber & ~mask) | truncatedPacketNumber;
        if (candidate + halfWindow <= expectedPacketNumber &&
            candidate <= QuicVarIntMaximum - window) {
            candidate += window;
        }
        else if (candidate > expectedPacketNumber + halfWindow && candidate >= window) {
            candidate -= window;
        }
        if (candidate > QuicVarIntMaximum) return STATUS_INTEGER_OVERFLOW;
    ...
    NTSTATUS QuicPacketIteratorNext(
        ...
        QuicBufferView* packet) noexcept
    {
        if (packet != nullptr) *packet = {};
        if (iterator == nullptr || header == nullptr || packet == nullptr ||
            (iterator->Datagram == nullptr && iterator->DatagramLength != 0)) {
            return STATUS_INVALID_PARAMETER;
        }
        if (iterator->Offset == iterator->DatagramLength) return STATUS_NOT_FOUND;
        if (iterator->Offset > iterator->DatagramLength) return STATUS_INVALID_NETWORK_RESPONSE;
        const UCHAR* current = iterator->Datagram + iterator->Offset;
        const SIZE_T remaining = iterator->DatagramLength - iterator->Offset;
        NTSTATUS status = QuicParsePacketHeader(
            current, remaining, iterator->ShortHeaderDestinationConnectionIdLength, header);
        if (!NT_SUCCESS(status)) return status;
