...
namespace ws
{
    namespace
    {
        ...
        _Must_inspect_result_
        bool TextEquals(http1::HttpText left, http1::HttpText right) noexcept
        {
        ...
        _Must_inspect_result_
        http1::HttpText TrimOptionalWhitespace(http1::HttpText text) noexcept
        {
        ...
        _Must_inspect_result_
        bool IsValidSubprotocolToken(http1::HttpText token) noexcept
        {
        ...
        NTSTATUS FindSelectedSubprotocol(
            ...
            _Out_ bool* present,
            _Out_ http1::HttpText* selected) noexcept
        {
            if (present != nullptr) {
                *present = false;
            }
        ...
        SIZE_T CountHeaders(
            _In_ const http1::HttpResponse& response,
            _In_ http1::HttpText name) noexcept
        {
            SIZE_T count = 0;
            if (response.Headers == nullptr && response.HeaderCount != 0) {
                return 0;
            ...
            for (SIZE_T index = 0; index < response.HeaderCount; ++index) {
                if (http1::TextEqualsIgnoreCase(response.Headers[index].Name, name)) {
                    ++count;
                }
            }
            return count;
        ...
        _Must_inspect_result_
        bool ParseWindowBits(http1::HttpText text, UCHAR* windowBits) noexcept
        {
            ...
            _Out_ PerMessageDeflateNegotiation& negotiated) noexcept
        {
            ...
            while (cursor <= value.Length) {
                ...
                http1::HttpText item = { value.Data + itemStart, cursor - itemStart };
                ...
                http1::HttpText extension = {
                    item.Data + extensionStart,
                    partCursor - extensionStart
                };
                extension = TrimOptionalWhitespace(extension);

                if (!http1::TextEqualsIgnoreCase(extension, http1::MakeText("permessage-deflate"))) {
                    return STATUS_INVALID_NETWORK_RESPONSE;
                }
                if (extensionSeen) {
                    return STATUS_INVALID_NETWORK_RESPONSE;
                ...
                while (partCursor < item.Length) {
                    ...
                    http1::HttpText name = { parameter.Data, equals };
                    name = TrimOptionalWhitespace(name);
                    http1::HttpText parameterValue = {};
                    ...
                    if (http1::TextEqualsIgnoreCase(name, http1::MakeText("client_no_context_takeover"))) {
                    ...
                    else if (http1::TextEqualsIgnoreCase(name, http1::MakeText("server_no_context_takeover"))) {
                    ...
                    else if (http1::TextEqualsIgnoreCase(name, http1::MakeText("client_max_window_bits"))) {
                    ...
                    else if (http1::TextEqualsIgnoreCase(name, http1::MakeText("server_max_window_bits"))) {
        ...
        NTSTATUS ValidateSelectedSubprotocol(
            _In_ http1::HttpText selected,
            _In_ http1::HttpText requested) noexcept
        {
        ...
        PerMessageDeflateNegotiation* negotiatedDeflate) noexcept
    {
        ...
        const http1::HttpHeader* extension = nullptr;
        SIZE_T extensionHeaderCount = 0;
        for (SIZE_T index = 0; index < headerCount; ++index) {
            if (!http1::TextEqualsIgnoreCase(headers[index].Name, headerName)) {
                continue;
            }
        ...
        PerMessageDeflateNegotiation* negotiatedDeflate) noexcept
    {
        ...
        if (response.StatusCode != 101 ||
            ...
            !response.HasHeaderValueToken(http1::MakeText("Connection"), http1::MakeText("Upgrade")) ||
            !response.HasHeaderValueToken(http1::MakeText("Upgrade"), http1::MakeText("websocket"))) {
            return STATUS_INVALID_NETWORK_RESPONSE;
        }

        const http1::HttpHeader* accept = nullptr;
        if (CountHeaders(response, http1::MakeText("Sec-WebSocket-Accept")) != 1 ||
            !response.FindHeader(http1::MakeText("Sec-WebSocket-Accept"), &accept) ||
            accept == nullptr) {
            return STATUS_INVALID_NETWORK_RESPONSE;
        }
        ...
        http1::HttpText negotiatedSubprotocol = {};
