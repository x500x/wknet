#include <wknet/engine/HttpEngine.h>
...
namespace session
{
        ...
        _Out_ SIZE_T* rawResponseLength) noexcept
    {
        ...
        *rawResponseLength = 0;
        ...
        if (pooledConnection.Http2 == nullptr) {
            ...
            if (!NT_SUCCESS(status)) {
                WKNET_DBG_PRINT("High-level HTTP/2 init failed: 0x%08X\r\n", static_cast<ULONG>(status));
        ...
        if (!NT_SUCCESS(status)) {
            WKNET_DBG_PRINT("High-level HTTP/2 request failed: 0x%08X\r\n", static_cast<ULONG>(status));
            return status;
        }
        ...
        if (!NT_SUCCESS(status)) {
            ...
            if (status == STATUS_BUFFER_TOO_SMALL && workspace.MaxResponseBytes != 0) {
                ...
                WKNET_DBG_PRINT("High-level HTTP/2 response failed: 0x%08X\r\n", static_cast<ULONG>(status));
            }
            return status;
        }
        ...
        status = DecodeContentWithWorkspace(
            ...
            &decoded);
        if (!NT_SUCCESS(status)) {
            WKNET_DBG_PRINT("High-level HTTP/2 content decode failed: 0x%08X\r\n", static_cast<ULONG>(status));
            return status;
        }
        ...
        _Out_ SIZE_T* rawResponseLength) noexcept
    {
        ...
        *rawResponseLength = 0;
        ...
        if (!NT_SUCCESS(status)) {
            WKNET_DBG_PRINT("High-level h2c Upgrade init failed: 0x%08X\r\n", static_cast<ULONG>(status));
            FreeNonPagedObject(h2Connection);
            return status;
        }
        pooledConnection.Http2 = h2Connection;
        ...
        status = pooledConnection.Http2->ReceiveResponse(
            *activeTransport,
            ...
            workspace.Http2HeaderScratch.Length);
        if (!NT_SUCCESS(status)) {
            WKNET_DBG_PRINT("High-level h2c Upgrade stream 1 failed: 0x%08X\r\n", static_cast<ULONG>(status));
            return status;
        }
        ...
        status = DecodeContentWithWorkspace(
            ...
            &decoded);
        if (!NT_SUCCESS(status)) {
            WKNET_DBG_PRINT("High-level h2c Upgrade content decode failed: 0x%08X\r\n", static_cast<ULONG>(status));
            return status;
        }
        ...
        _In_opt_ KH_ASYNC_OPERATION cancellationOperation) noexcept
    {
        ...
        if (NT_SUCCESS(status)) {
            ...
            for (SIZE_T addressIndex = 0; addressIndex < remoteAddressCount; ++addressIndex) {
                ...
                lastStatus = status;
                WKNET_DBG_PRINT("HttpEngine socket connect attempt failed: 0x%08X index=%Iu family=%u\r\n",
                    static_cast<ULONG>(status),
                    addressIndex,
        ...
        _In_opt_ KH_ASYNC_OPERATION cancellationOperation) noexcept
    {
        ...
        status = ConnectTlsOnExistingSocket(
            ...
            nullptr);
        if (NT_SUCCESS(status)) {
            WKNET_DBG_PRINT("HttpEngine TLS1.2 confirmed after version negotiation\r\n");
            return STATUS_SUCCESS;
        }
        ...
        _In_opt_ KH_ASYNC_OPERATION cancellationOperation) noexcept
    {
        ...
        if (h2Negotiated || h2ExplicitlyRequested) {
            ...
            if (!TextEqualsLiteral(negotiatedAlpn, negotiatedAlpnLength, "h2")) {
                WKNET_DBG_PRINT("High-level HTTP/2 ALPN not negotiated: %.*s\r\n",
    ...
    http1::HttpText FindLocationHeader(const http1::HttpResponse& response) noexcept
    {
        ...
        if (response.FindHeader(http1::MakeText("Location"), &header) && header != nullptr) {
            return header->Value;
        }
    ...
    bool IsAbsoluteHttpLocation(http1::HttpText location) noexcept
    {
    ...
    http1::HttpText TrimRedirectLocation(http1::HttpText location) noexcept
    {
    ...
    bool HasUriScheme(http1::HttpText location) noexcept
    {
    ...
    bool IsSchemeRelativeLocation(http1::HttpText location) noexcept
    {
    ...
    bool IsPathAbsoluteLocation(http1::HttpText location) noexcept
    {
        return location.Data != nullptr &&
            location.Length >= 1 &&
            location.Data[0] == '/';
    ...
    void SplitRedirectReference(
        ...
        _Out_ http1::HttpText* path,
        _Out_ http1::HttpText* query,
        _Out_ http1::HttpText* fragment,
        _Out_ bool* hasQuery,
    ...
    NTSTATUS AppendRedirectPathAndSuffix(
        ...
        _Inout_ SIZE_T* offset) noexcept
    {
        http1::HttpText referencePath = {};
        http1::HttpText referenceQuery = {};
        http1::HttpText referenceFragment = {};
        ...
        const http1::HttpText basePath = { request.Path, baseQueryIndex };
        const http1::HttpText baseQuery =
            baseQueryIndex < request.PathLength ?
            http1::HttpText{ request.Path + baseQueryIndex, request.PathLength - baseQueryIndex } :
            http1::HttpText{};
        ...
        *offset = pathStartOffset + normalizedPathLength;
        ...
        const http1::HttpText fragmentToAppend =
        ...
        _Out_ SIZE_T* destinationLength) noexcept
    {
        ...
        if (IsAbsoluteHttpLocation(location)) {
            ...
            if (FindRedirectChar(location.Data, location.Length, '#') == location.Length) {
                const http1::HttpText baseFragment = BaseRequestFragment(request);
