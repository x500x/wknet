...
namespace wknet::net {
    namespace
    {
        ...
        ULONG AddressLengthForFamily(USHORT family) noexcept
        {
            if (family == AF_INET) {
        ...
        void CopyAddress(
            _Out_ SOCKADDR_STORAGE* destination,
        ...
        bool TakeQueuedCompletion(
            test::WskDatagramTestReceiveCompletion* completion) noexcept
        {
        ...
        NTSTATUS TestSubmitReceive(WskDatagramSocket* socket) noexcept
        {
            ...
            socket->TestCompleteReceive(
                completion.Data,
                completion.DataLength,
                completion.RemoteAddress,
                completion.RemoteAddressLength,
                completion.Status,
                false);
...
#else
        NTSTATUS CompleteSynchronousWskRequest(
            NTSTATUS requestStatus,
            WskSyncIrpContext* context,
            SIZE_T* information = nullptr) noexcept
        {
            const NTSTATUS status = WskSyncCompleteIrp(
                requestStatus,
                context,
    ...
    namespace test
    {
        ...
        void QueueReceiveCompletion(const WskDatagramTestReceiveCompletion& completion) noexcept
        {
            ...
            const SIZE_T tail = (g_testProvider.QueueHead + g_testProvider.QueueCount) %
                TestCompletionQueueCapacity;
    ...
    bool WskDatagramSocket::AddressMatchesFamily(const SOCKADDR* address) const noexcept
    {
        return address != nullptr &&
            AddressLength() != 0 &&
            address->sa_family == addressFamily_;
    }
    ...
    NTSTATUS WskDatagramSocket::Open(WskClient& client, USHORT addressFamily) noexcept
    {
        ...
        status = provider->WskSocket(
            client.ProviderClient(),
            static_cast<ADDRESS_FAMILY>(addressFamily_),
            SOCK_DGRAM,
            IPPROTO_UDP,
        ...
        if (dispatch_ == nullptr || dispatch_->WskBind == nullptr ||
            dispatch_->WskSendTo == nullptr || dispatch_->WskReceiveFrom == nullptr ||
            dispatch_->Basic.WskCloseSocket == nullptr) {
    ...
    NTSTATUS WskDatagramSocket::Bind(const SOCKADDR* localAddress) noexcept
    {
        ...
        if (dispatch_->WskGetLocalAddress != nullptr) {
            ...
            status = dispatch_->WskGetLocalAddress(
                socket_,
                reinterpret_cast<SOCKADDR*>(&localAddress_),
                context->Irp);
            status = CompleteSynchronousWskRequest(status, context);
            if (!NT_SUCCESS(status)) {
                return status;
    ...
    NTSTATUS WskDatagramSocket::ConnectPeer(const SOCKADDR* remoteAddress) noexcept
    {
        ...
        if (endpointState_ == WskDatagramEndpointState::Created) {
            ...
            const NTSTATUS bindStatus = Bind(
                reinterpret_cast<const SOCKADDR*>(&bindScratchAddress_));
    ...
    NTSTATUS WskDatagramSocket::CompleteReceiveRecord(
        NTSTATUS status,
        SIZE_T information,
...
#if defined(WKNET_USER_MODE_TEST)
    void WskDatagramSocket::TestCompleteReceive(
        const void* data,
        SIZE_T dataLength,
...
#else
    NTSTATUS WskDatagramSocket::ReceiveCompletionRoutine(
        ...
        PVOID context) noexcept
    {
        ...
        if (socket != nullptr) {
            (void)socket->CompleteReceiveRecord(
                irp->IoStatus.Status,
                static_cast<SIZE_T>(irp->IoStatus.Information),
                socket->AddressLength(),
                socket->receiveGeneration_);
        }
    ...
    NTSTATUS WskDatagramSocket::StartReceive(void* data, SIZE_T length) noexcept
    {
        ...
        if (data == nullptr || length == 0 ||
            length > WKNET_HARD_MAX_QUIC_UDP_PAYLOAD_BYTES ||
            length > MAXULONG) {
            return STATUS_INVALID_PARAMETER;
        }
...
#if defined(WKNET_USER_MODE_TEST)
        {
            ...
            if (endpointState_ != WskDatagramEndpointState::Bound &&
                endpointState_ != WskDatagramEndpointState::Connected) {
                return STATUS_INVALID_DEVICE_STATE;
            }
        ...
        NTSTATUS status = dispatch_->WskReceiveFrom(
            socket_,
            &receiveBuffer_,
            0,
            reinterpret_cast<SOCKADDR*>(&receiveRemoteAddress_),
            &receiveControlLength_,
            nullptr,
            &receiveControlFlags_,
            receiveIrp_);
        ...
        if (!alreadyCompleted && NT_SUCCESS(status)) {
            status = CompleteReceiveRecord(
                status,
                static_cast<SIZE_T>(receiveIrp_->IoStatus.Information),
                AddressLength(),
                generation);
    ...
    NTSTATUS WskDatagramSocket::CompleteReceive(
        ...
        WskDatagramReceiveResult* result) noexcept
    {
...
#if defined(WKNET_USER_MODE_TEST)
        {
            ...
            if (receiveState_ != WskDatagramReceiveState::Completed) {
                ...
                if (timeoutMilliseconds == MAXULONG) {
                    ...
                    signaled = receiveEvent_.wait_for(
                        lock,
        ...
        if (timedOut) {
            ...
            receiveEvent_.wait(lock, [this]() noexcept { return receiveEventSignaled_; });
#else
            const NTSTATUS drainStatus = KeWaitForSingleObject(
                &receiveEvent_, Executive, KernelMode, FALSE, nullptr);
        ...
        if (NT_SUCCESS(result->Status)) {
            if (result->BytesReceived > receiveCapacity_ ||
                result->RemoteAddressLength > sizeof(SOCKADDR_STORAGE) ||
        ...
        if (!NT_SUCCESS(status)) {
            ...
            WKNET_TRACE_CORRELATED(
                ...
                &correlation,
                "net.datagram.receive.failed status=0x%08X",
                static_cast<ULONG>(status));
        }
        return status;
    }

    NTSTATUS WskDatagramSocket::Send(
        const void* data,
        SIZE_T length,
    ...
    NTSTATUS WskDatagramSocket::SendTo(
        const void* data,
        SIZE_T length,
        ...
        SIZE_T* bytesSent) noexcept
    {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            WKNET_TRACE_CORRELATED(
                ...
                ::wknet::TraceLevel::Error,
                &correlation,
                "net.datagram.send.failed status=0x%08X bytes=%Iu",
                static_cast<ULONG>(status),
                length);
        }
        return status;
    ...
    NTSTATUS WskDatagramSocket::Close() noexcept
    {
        ...
        WKNET_TRACE_CORRELATED(
            ...
            ::wknet::TraceLevel::Info,
            &correlation,
            "net.datagram.close_completed status=0x%08X",
    ...
    NTSTATUS WskDatagramSocketCreate(
        WskClient* client,
        USHORT addressFamily,
        WskDatagramSocket** socket) noexcept
    {
        if (socket != nullptr) {
            *socket = nullptr;
        }
        ...
        *socket = created;
        ...
        WKNET_TRACE_CORRELATED(
            ...
            ::wknet::TraceLevel::Info,
            &correlation,
            "net.datagram.opened family=%u",
            static_cast<ULONG>(addressFamily));
        return STATUS_SUCCESS;
    }

    NTSTATUS WskDatagramSocketBind(
        WskDatagramSocket* socket,
        const SOCKADDR* localAddress) noexcept
    {
        return socket != nullptr ? socket->Bind(localAddress) : STATUS_INVALID_PARAMETER;
    }

    NTSTATUS WskDatagramSocketConnectPeer(
        WskDatagramSocket* socket,
        const SOCKADDR* remoteAddress) noexcept
    {
        return socket != nullptr ? socket->ConnectPeer(remoteAddress) : STATUS_INVALID_PARAMETER;
    }

    NTSTATUS WskDatagramSocketStartReceive(
        WskDatagramSocket* socket,
        void* data,
    ...
    NTSTATUS WskDatagramSocketCompleteReceive(
        WskDatagramSocket* socket,
        ULONG timeoutMilliseconds,
        WskDatagramReceiveResult* result) noexcept
    {
        return socket != nullptr
            ? socket->CompleteReceive(timeoutMilliseconds, result)
            : STATUS_INVALID_PARAMETER;
    }

    NTSTATUS WskDatagramSocketSend(
        WskDatagramSocket* socket,
        const void* data,
        SIZE_T length,
        SIZE_T* bytesSent) noexcept
    {
        return socket != nullptr
            ? socket->Send(data, length, bytesSent)
            : STATUS_INVALID_PARAMETER;
    }

    NTSTATUS WskDatagramSocketSendTo(
        WskDatagramSocket* socket,
        const void* data,
        SIZE_T length,
        const SOCKADDR* remoteAddress,
    ...
    {
        return socket != nullptr
            ? socket->SendTo(data, length, remoteAddress, bytesSent)
            : STATUS_INVALID_PARAMETER;
    }

    NTSTATUS WskDatagramSocketGetLocalAddress(
        const WskDatagramSocket* socket,
        SOCKADDR_STORAGE* localAddress) noexcept
    {
        return socket != nullptr ? socket->GetLocalAddress(localAddress) : STATUS_INVALID_PARAMETER;
    }

    NTSTATUS WskDatagramSocketGetRemoteAddress(
        const WskDatagramSocket* socket,
    ...
    void WskDatagramSocketSetConnectionId(
        WskDatagramSocket* socket,
        ULONGLONG connectionId) noexcept
    {
        if (socket != nullptr) {
