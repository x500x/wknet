...
class WskDatagramSocket final
{
  public:
    ...
    NTSTATUS SetReceiveNotification(WskDatagramReceiveNotification notification, void *context) noexcept;
    ...
    NTSTATUS SendTo(const void *data, SIZE_T length, const SOCKADDR *remoteAddress, SIZE_T *bytesSent) noexcept;
...
#if defined(WKNET_USER_MODE_TEST)
    void TestCompleteReceive(const void *data, SIZE_T dataLength, const SOCKADDR_STORAGE &remoteAddress,
                             ULONG remoteAddressLength, NTSTATUS status, bool dispatchCompletion) noexcept;
#endif
...
#if !defined(WKNET_USER_MODE_TEST)
    _Function_class_(IO_COMPLETION_ROUTINE) static NTSTATUS
        ReceiveCompletionRoutine(_In_ PDEVICE_OBJECT deviceObject, _In_ PIRP irp, _In_opt_ PVOID context) noexcept;
#endif
