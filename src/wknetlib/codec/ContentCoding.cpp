...
#include "codec/DeflateDecoder.h"
...
namespace codec
{
    ...
    namespace
    {
        ...
        bool ResolveMaterial(
            const DecodeMaterials* materials,
            Coding coding,
            ExternalMaterial* material) noexcept
        {
            if (material == nullptr) {
                return false;
            }
            *material = {};
            material->CodingKind = coding;
            ...
            if (materials->Items != nullptr) {
                for (SIZE_T index = 0; index < materials->ItemCount; ++index) {
                    const ExternalMaterial& item = materials->Items[index];
                    if (item.CodingKind == coding) {
                        *material = item;
                        return true;
                    }
                }
        ...
        NTSTATUS DecodeRawDeflate(
            ...
            SIZE_T* decodedLength) noexcept
        {
            if (!ContentCoding::DeflateRuntimeAvailable()) {
                if (decodedLength != nullptr) {
                    *decodedLength = 0;
                }
                return STATUS_NOT_SUPPORTED;
        ...
        _Must_inspect_result_
        bool HasTransform(Coding coding) noexcept
        {
    ...
    bool ContentCoding::DeflateRuntimeAvailable() noexcept
    {
#if defined(WKNET_USER_MODE_TEST)
    ...
    NTSTATUS ContentCoding::DecodeOne(
        Coding coding,
        const char* source,
        SIZE_T sourceLength,
        char* destination,
        ...
        const DecodeMaterials* materials) noexcept
    {
        ...
        case Coding::DictionaryCompressedBrotli:
        ...
        case Coding::DictionaryCompressedZstd:
            if (!ResolveMaterial(materials, coding, &material) ||
                material.Dictionary == nullptr ||
                material.DictionaryLength == 0) {
                return STATUS_NOT_SUPPORTED;
    ...
    NTSTATUS ContentCoding::DecodeChainReverse(
        ...
        DecodeResult& result) noexcept
    {
        ...
        for (SIZE_T reverseIndex = codingCount; reverseIndex > 0; --reverseIndex) {
            const Coding coding = codings[reverseIndex - 1];
...
{
    return wknet::codec::BrotliAlloc(nullptr, size);
}

extern "C" void WknetBrotliFree(void* address)
{
    wknet::codec::BrotliFree(nullptr, address);
}
