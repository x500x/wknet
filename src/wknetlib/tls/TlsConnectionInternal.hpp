    ...
    namespace connection_detail
    {
            ...
            _In_ const TlsHandshakeMessageView& handshake) noexcept
        {
            ...
            WKNET_TRACE(::wknet::ComponentTls, ::wknet::TraceLevel::Error,
                "tls12.server_key_exchange.parse_failed status=0x%08X cipher=0x%04X key_exchange=%u type=%u body_bytes=%Iu curve=%u group=%u point_bytes=%u signature=0x%04X signature_bytes=%u offset=%Iu",
        ...
        inline NTSTATUS LogTls13Failure(_In_z_ const char* stage, NTSTATUS status) noexcept
        {
            ...
            WKNET_TRACE(::wknet::ComponentTls, ::wknet::TraceLevel::Error, "tls13.stage.failed stage=%s status=0x%08X", stage, static_cast<ULONG>(status));
#else
