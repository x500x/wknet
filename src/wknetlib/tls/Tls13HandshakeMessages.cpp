...
const TlsCipherSuite DefaultCipherSuites[] = {TlsCipherSuite::TlsAes128GcmSha256, TlsCipherSuite::TlsAes256GcmSha384,
                                              TlsCipherSuite::TlsChaCha20Poly1305Sha256};

const TlsNamedGroup DefaultNamedGroups[] = {
    TlsNamedGroup::X25519,    TlsNamedGroup::X448,      TlsNamedGroup::Secp256r1, TlsNamedGroup::Secp384r1,
    TlsNamedGroup::Secp521r1, TlsNamedGroup::Ffdhe2048, TlsNamedGroup::Ffdhe3072, TlsNamedGroup::Ffdhe4096,
    TlsNamedGroup::Ffdhe6144, TlsNamedGroup::Ffdhe8192};

const TlsSignatureScheme DefaultSignatureSchemes[] = {TlsSignatureScheme::RsaPssRsaeSha256,
...
_Must_inspect_result_ bool IsValidBuffer(_In_reads_bytes_opt_(length) const UCHAR *data, SIZE_T length) noexcept
{
    return length == 0 || data != nullptr;
}

_Must_inspect_result_ bool HasCapacity(SIZE_T capacity, SIZE_T offset, SIZE_T writeLength) noexcept
{
    return offset <= capacity && writeLength <= (capacity - offset);
}

_Must_inspect_result_ SIZE_T StringLength(_In_z_ const char *value) noexcept
{
    SIZE_T length = 0;
    if (value == nullptr)
    {
...
_Must_inspect_result_ NTSTATUS WriteByte(UCHAR value, _Out_writes_bytes_(capacity) UCHAR *destination, SIZE_T capacity,
...
_Must_inspect_result_ NTSTATUS WriteUint16(USHORT value, _Out_writes_bytes_(capacity) UCHAR *destination,
                                           SIZE_T capacity, _Inout_ SIZE_T *offset) noexcept
{
...
_Must_inspect_result_ NTSTATUS WriteUint24(ULONG value, _Out_writes_bytes_(capacity) UCHAR *destination,
                                           SIZE_T capacity, _Inout_ SIZE_T *offset) noexcept
{
    if (value > TlsMaxHandshakeMessageLength || destination == nullptr || offset == nullptr ||
...
_Must_inspect_result_ NTSTATUS WriteUint32(ULONG value, _Out_writes_bytes_(capacity) UCHAR *destination,
                                           SIZE_T capacity, _Inout_ SIZE_T *offset) noexcept
{
...
_Must_inspect_result_ NTSTATUS WriteBytes(_In_reads_bytes_opt_(length) const UCHAR *source, SIZE_T length,
                                          _Out_writes_bytes_(capacity) UCHAR *destination, SIZE_T capacity,
                                          _Inout_ SIZE_T *offset) noexcept
{
    if (!IsValidBuffer(source, length) || destination == nullptr || offset == nullptr ||
...
_Must_inspect_result_ NTSTATUS ReadByte(_In_reads_bytes_(capacity) const UCHAR *source, SIZE_T capacity,
                                        _Inout_ SIZE_T *offset, _Out_ UCHAR *value) noexcept
{
...
_Must_inspect_result_ NTSTATUS ReadUint16(_In_reads_bytes_(capacity) const UCHAR *source, SIZE_T capacity,
                                          _Inout_ SIZE_T *offset, _Out_ USHORT *value) noexcept
{
...
_Must_inspect_result_ NTSTATUS ReadUint24(_In_reads_bytes_(capacity) const UCHAR *source, SIZE_T capacity,
                                          _Inout_ SIZE_T *offset, _Out_ SIZE_T *value) noexcept
{
    ...
    *value = (static_cast<SIZE_T>(source[*offset]) << 16) | (static_cast<SIZE_T>(source[*offset + 1]) << 8) |
             source[*offset + 2];
    *offset += 3;
    return STATUS_SUCCESS;
}

_Must_inspect_result_ NTSTATUS ReadUint32(_In_reads_bytes_(capacity) const UCHAR *source, SIZE_T capacity,
                                          _Inout_ SIZE_T *offset, _Out_ ULONG *value) noexcept
{
    ...
    *value = (static_cast<ULONG>(source[*offset]) << 24) | (static_cast<ULONG>(source[*offset + 1]) << 16) |
             (static_cast<ULONG>(source[*offset + 2]) << 8) | source[*offset + 3];
    *offset += 4;
    return STATUS_SUCCESS;
}

_Must_inspect_result_ NTSTATUS ReadBytes(_In_reads_bytes_(capacity) const UCHAR *source, SIZE_T capacity,
                                         _Inout_ SIZE_T *offset, SIZE_T length,
...
_Must_inspect_result_ NTSTATUS ValidateHandshakeType(const TlsHandshakeMessageView &message,
...
_Must_inspect_result_ bool MemoryEquals(const UCHAR *left, const UCHAR *right, SIZE_T length) noexcept
{
    UCHAR diff = 0;
...
_Must_inspect_result_ bool IsAsciiLetter(UCHAR value) noexcept
{
    return (value >= 'A' && value <= 'Z') || (value >= 'a' && value <= 'z');
}

_Must_inspect_result_ bool IsAsciiDigit(UCHAR value) noexcept
{
    return value >= '0' && value <= '9';
}

_Must_inspect_result_ bool IsValidSniDnsName(_In_reads_(length) const char *name, SIZE_T length) noexcept
{
...
_Must_inspect_result_ NTSTATUS BuildServerNameExtension(const Tls13ClientHelloOptions &options, UCHAR *destination,
                                                        SIZE_T capacity, SIZE_T *offset) noexcept
{
    ...
    if (options.ServerNameLength > 0xffff - 5 || !IsValidSniDnsName(options.ServerName, options.ServerNameLength))
    {
    ...
    if (NT_SUCCESS(status))
    {
        status = WriteBytes(reinterpret_cast<const UCHAR *>(options.ServerName), options.ServerNameLength, destination,
                            capacity, offset);
    }
    if (!NT_SUCCESS(status))
    {
...
_Must_inspect_result_ NTSTATUS BuildUint16VectorExtension(USHORT extensionType, const TValue *values, SIZE_T valueCount,
                                                          UCHAR *destination, SIZE_T capacity, SIZE_T *offset) noexcept
{
...
_Must_inspect_result_ NTSTATUS BuildSupportedVersionsExtension(UCHAR *destination, SIZE_T capacity,
...
_Must_inspect_result_ NTSTATUS BuildPskModesExtension(UCHAR *destination, SIZE_T capacity, SIZE_T *offset) noexcept
{
...
_Must_inspect_result_ NTSTATUS BuildKeyShareExtension(const Tls13ClientHelloOptions &options, UCHAR *destination,
                                                      SIZE_T capacity, SIZE_T *offset) noexcept
{
    ...
    for (SIZE_T index = 0; NT_SUCCESS(status) && index < options.KeyShareCount; ++index)
    {
        ...
        if (share.KeyExchange == nullptr || share.KeyExchangeLength == 0 || share.KeyExchangeLength > 0xffff)
        {
            return STATUS_INVALID_PARAMETER;
        }
...
_Must_inspect_result_ NTSTATUS BuildAlpnExtension(const Tls13ClientHelloOptions &options, UCHAR *destination,
                                                  SIZE_T capacity, SIZE_T *offset) noexcept
{
    ...
    for (SIZE_T index = 0; NT_SUCCESS(status) && index < options.AlpnProtocolCount; ++index)
    {
        ...
        if (NT_SUCCESS(status))
        {
            status = WriteBytes(reinterpret_cast<const UCHAR *>(protocol.Name), protocol.NameLength, destination,
                                capacity, offset);
        }
    }
    return status;
...
_Must_inspect_result_ NTSTATUS BuildEarlyDataExtension(bool enabled, UCHAR *destination, SIZE_T capacity,
                                                       SIZE_T *offset) noexcept
{
    if (!enabled)
    {
...
_Must_inspect_result_ bool IsReservedCustomExtension(USHORT extensionType) noexcept
{
...
_Must_inspect_result_ NTSTATUS BuildCustomExtensions(const Tls13ClientHelloOptions &options, UCHAR *destination,
                                                     SIZE_T capacity, SIZE_T *offset) noexcept
{
    if (options.CustomExtensionCount == 0)
    {
        return options.CustomExtensions == nullptr ? STATUS_SUCCESS : STATUS_INVALID_PARAMETER;
    }
    if (options.CustomExtensions == nullptr || options.CustomExtensionCount > 64)
    {
        return STATUS_INVALID_PARAMETER;
    }

    NTSTATUS status = STATUS_SUCCESS;
    for (SIZE_T index = 0; index < options.CustomExtensionCount; ++index)
    {
        const Tls13ExtensionView &extension = options.CustomExtensions[index];
        if (IsReservedCustomExtension(extension.Type) || extension.Length > 0xffff ||
            !IsValidBuffer(extension.Data, extension.Length))
        {
            return STATUS_INVALID_PARAMETER;
        }
        for (SIZE_T prior = 0; prior < index; ++prior)
        {
            if (options.CustomExtensions[prior].Type == extension.Type)
            {
                return STATUS_INVALID_PARAMETER;
            }
        }

        status = WriteUint16(extension.Type, destination, capacity, offset);
        if (NT_SUCCESS(status))
        {
            status = WriteUint16(static_cast<USHORT>(extension.Length), destination, capacity, offset);
        }
        if (NT_SUCCESS(status))
        {
            status = WriteBytes(extension.Data, extension.Length, destination, capacity, offset);
        }
        if (!NT_SUCCESS(status))
        {
            return status;
...
_Must_inspect_result_ NTSTATUS BuildPreSharedKeyExtension(const Tls13ClientHelloOptions &options, UCHAR *destination,
                                                          SIZE_T capacity, SIZE_T *offset) noexcept
{
...
_Must_inspect_result_ SIZE_T HashLength(crypto::HashAlgorithm algorithm) noexcept
{
    switch (algorithm)
    {
...
_Must_inspect_result_ NTSTATUS FindExtension(const UCHAR *extensions, SIZE_T extensionsLength, USHORT extensionType,
                                             _Outptr_result_bytebuffer_(*extensionLength) const UCHAR **extension,
                                             _Out_ SIZE_T *extensionLength, _Out_opt_ bool *found) noexcept
{
    ...
    while (offset < extensionsLength)
    {
        ...
        const USHORT currentType =
            static_cast<USHORT>((static_cast<USHORT>(extensions[offset]) << 8) | extensions[offset + 1]);
        const SIZE_T currentLength = (static_cast<SIZE_T>(extensions[offset + 2]) << 8) | extensions[offset + 3];
        offset += 4;
...
_Must_inspect_result_ NTSTATUS ValidateNoDuplicateExtensions(_In_reads_bytes_(extensionsLength) const UCHAR *extensions,
                                                             SIZE_T extensionsLength) noexcept
{
    ...
    while (offset < extensionsLength)
    {
        ...
        while (priorOffset < currentExtensionOffset)
        {
            ...
            const USHORT priorType =
                static_cast<USHORT>((static_cast<USHORT>(extensions[priorOffset]) << 8) | extensions[priorOffset + 1]);
            const SIZE_T priorLength =
                (static_cast<SIZE_T>(extensions[priorOffset + 2]) << 8) | extensions[priorOffset + 3];
            priorOffset += 4;
...
_Must_inspect_result_ NTSTATUS ParseAlpnExtension(const UCHAR *data, SIZE_T length, const char **alpn,
                                                  SIZE_T *alpnLength) noexcept
{
    ...
    *alpnLength = 0;
    ...
    if (!NT_SUCCESS(status) || protocolLength == 0 || protocolLength > length - offset ||
        offset + protocolLength != length)
    {
        return NT_SUCCESS(status) ? STATUS_INVALID_NETWORK_RESPONSE : status;
    }
...
_Must_inspect_result_ NTSTATUS ComputeFinishedData(const TlsContext &context, bool clientFinished,
                                                   const UCHAR *transcriptHash, SIZE_T transcriptHashLength,
                                                   UCHAR *output, SIZE_T outputLength) noexcept
{
    ...
    NTSTATUS status =
        context.DeriveTls13FinishedKey(clientFinished, finishedKey.Get(), finishedKey.Count(), &finishedKeyLength);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    ...
    SIZE_T hmacLength = 0;
    status = crypto::CngProvider::Hmac(Tls13HandshakeMessages::HashForCipherSuite(context.CipherSuite()),
                                       finishedKey.Get(), finishedKeyLength, transcriptHash, transcriptHashLength,
                                       output, outputLength, &hmacLength);
...
{
    return cipherSuite == TlsCipherSuite::TlsAes256GcmSha384 ? crypto::HashAlgorithm::Sha384
                                                             : crypto::HashAlgorithm::Sha256;
}

bool Tls13HandshakeMessages::IsSupportedCipherSuite(TlsCipherSuite cipherSuite) noexcept
{
    return cipherSuite == TlsCipherSuite::TlsAes128GcmSha256 || cipherSuite == TlsCipherSuite::TlsAes256GcmSha384 ||
           cipherSuite == TlsCipherSuite::TlsChaCha20Poly1305Sha256 ||
           cipherSuite == TlsCipherSuite::TlsAes128CcmSha256 || cipherSuite == TlsCipherSuite::TlsAes128Ccm8Sha256;
}

namespace
{
_Must_inspect_result_ bool ContainsCipherSuite(_In_reads_(cipherSuiteCount) const TlsCipherSuite *cipherSuites,
                                               SIZE_T cipherSuiteCount, TlsCipherSuite selected) noexcept
{
...
_Must_inspect_result_ bool ContainsNamedGroup(_In_reads_(namedGroupCount) const TlsNamedGroup *namedGroups,
                                              SIZE_T namedGroupCount, TlsNamedGroup selected) noexcept
{
...
_Must_inspect_result_ bool ContainsKeyShareGroup(_In_reads_(keyShareCount) const Tls13KeyShareEntry *keyShares,
                                                 SIZE_T keyShareCount, TlsNamedGroup selected) noexcept
{
...
NTSTATUS Tls13HandshakeMessages::EncodeClientHello(TlsContext &context, const Tls13ClientHelloOptions &options,
                                                   UCHAR *destination, SIZE_T destinationCapacity,
                                                   SIZE_T *bytesWritten) noexcept
{
    ...
    if (context.Protocol() != TlsProtocol::Tls13 || (options.ServerName == nullptr && options.ServerNameLength != 0))
    {
        return STATUS_INVALID_PARAMETER;
    }

    const TlsCipherSuite *cipherSuites = options.CipherSuites != nullptr ? options.CipherSuites : DefaultCipherSuites;
    const SIZE_T cipherSuiteCount = options.CipherSuites != nullptr
                                        ? options.CipherSuiteCount
    ...
    const SIZE_T namedGroupCount = options.NamedGroups != nullptr
                                       ? options.NamedGroupCount
    ...
    const SIZE_T signatureSchemeCount = options.SignatureSchemes != nullptr
                                            ? options.SignatureSchemeCount
    ...
    if (NT_SUCCESS(status))
    {
        status = BuildUint16VectorExtension(ExtensionSupportedGroups, namedGroups, namedGroupCount, extensions.Get(),
                                            extensions.Count(), &extensionOffset);
    }
    if (NT_SUCCESS(status))
    {
        status = BuildUint16VectorExtension(ExtensionSignatureAlgorithms, signatureSchemes, signatureSchemeCount,
                                            extensions.Get(), extensions.Count(), &extensionOffset);
    }
    if (NT_SUCCESS(status))
    {
        status = BuildUint16VectorExtension(ExtensionSignatureAlgorithmsCert, signatureSchemes, signatureSchemeCount,
                                            extensions.Get(), extensions.Count(), &extensionOffset);
    }
    if (NT_SUCCESS(status))
    {
    ...
    {
        status =
            BuildEarlyDataExtension(options.OfferEarlyData, extensions.Get(), extensions.Count(), &extensionOffset);
    }
    if (NT_SUCCESS(status))
    {
        status = BuildCustomExtensions(options, extensions.Get(), extensions.Count(), &extensionOffset);
    }
    if (NT_SUCCESS(status))
    {
    ...
    status =
        WriteByte(static_cast<UCHAR>(TlsHandshakeType::ClientHello), destination, destinationCapacity, &outputOffset);
    if (NT_SUCCESS(status))
    {
...
NTSTATUS Tls13HandshakeMessages::FindExtensionView(const UCHAR *extensions, SIZE_T extensionsLength,
                                                   USHORT extensionType, Tls13ExtensionView &extension,
                                                   bool *found) noexcept
{
    extension = {};
    if (found != nullptr)
    ...
    const NTSTATUS status = FindExtension(extensions, extensionsLength, extensionType, &data, &length, &localFound);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
...
NTSTATUS Tls13HandshakeMessages::ParseServerHello(TlsContext &context, const TlsHandshakeMessageView &message,
                                                  Tls13ServerHelloView &serverHello) noexcept
{
    ...
    serverHello.LegacyVersion = {static_cast<UCHAR>((legacyVersion >> 8) & 0xff),
                                 static_cast<UCHAR>(legacyVersion & 0xff)};
    ...
    serverHello.IsHelloRetryRequest =
        MemoryEquals(serverHello.Random, HelloRetryRequestRandom, sizeof(HelloRetryRequestRandom));
    ...
    bool found = false;
    status = FindExtension(serverHello.Extensions, serverHello.ExtensionsLength, ExtensionSupportedVersions, &extension,
                           &extensionLength, &found);
    ...
    status = FindExtension(serverHello.Extensions, serverHello.ExtensionsLength, ExtensionKeyShare, &extension,
                           &extensionLength, &found);
    if (!NT_SUCCESS(status))
    {
        return status;
    ...
    else
    {
        ...
        if (NT_SUCCESS(status))
        {
            status = ReadBytes(extension, extensionLength, &extensionOffset, keyExchangeLength,
    ...
    status = FindExtension(serverHello.Extensions, serverHello.ExtensionsLength, ExtensionPreSharedKey, &extension,
                           &extensionLength, &found);
    if (!NT_SUCCESS(status))
    {
        return status;
...
NTSTATUS Tls13HandshakeMessages::ValidateServerHelloOffer(const Tls13ServerHelloView &serverHello,
                                                          const Tls13ClientHelloOptions &clientHello) noexcept
{
    ...
    const SIZE_T cipherSuiteCount = clientHello.CipherSuites != nullptr
    ...
    if (serverHello.IsHelloRetryRequest)
    {
        ...
        const SIZE_T namedGroupCount = clientHello.NamedGroups != nullptr
...
NTSTATUS Tls13HandshakeMessages::ValidateSelectedPskIdentity(const Tls13ServerHelloView &serverHello,
                                                             SIZE_T offeredPskIdentityCount) noexcept
{
    ...
    if (offeredPskIdentityCount == 0 || offeredPskIdentityCount > 0xffff ||
...
NTSTATUS Tls13HandshakeMessages::ComputePskBinder(const TlsContext &context, const UCHAR *resumptionSecret,
                                                  SIZE_T resumptionSecretLength, const UCHAR *partialClientHelloHash,
                                                  SIZE_T partialClientHelloHashLength, UCHAR *binder,
                                                  SIZE_T binderCapacity, SIZE_T *binderLength) noexcept
{
    if (binderLength != nullptr)
    {
        *binderLength = 0;
    }
    if (resumptionSecret == nullptr || resumptionSecretLength == 0 || partialClientHelloHash == nullptr ||
        partialClientHelloHashLength == 0 || binder == nullptr || binderLength == nullptr ||
    ...
    NTSTATUS status = crypto::CngProvider::HkdfExtract(algorithm, nullptr, 0, resumptionSecret, resumptionSecretLength,
                                                       earlySecret.Get(), earlySecret.Count(), &earlySecretLength);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    ...
    status = crypto::CngProvider::Hash(algorithm, nullptr, 0, emptyHash.Get(), emptyHash.Count(), &emptyHashLength);
    ...
    status = DeriveSecret(algorithm, earlySecret.Get(), earlySecretLength, "res binder", emptyHash.Get(),
                          emptyHashLength, binderKey.Get(), digestLength);
    ...
    status = HkdfExpandLabel(algorithm, binderKey.Get(), digestLength, "finished", nullptr, 0, finishedKey.Get(),
                             digestLength);
    ...
    SIZE_T written = 0;
    status = crypto::CngProvider::Hmac(algorithm, finishedKey.Get(), digestLength, partialClientHelloHash,
                                       partialClientHelloHashLength, binder, binderCapacity, &written);
    RtlSecureZeroMemory(finishedKey.Get(), finishedKey.Count());
    if (NT_SUCCESS(status))
    {
...
NTSTATUS Tls13HandshakeMessages::FindPskBinderTranscriptLength(const UCHAR *clientHello, SIZE_T clientHelloLength,
                                                               SIZE_T *transcriptLength) noexcept
{
    ...
    if (parsed.Type != TlsHandshakeType::ClientHello || parsed.BytesConsumed != clientHelloLength ||
        parsed.Body == nullptr || parsed.BodyLength < 42)
    {
        return STATUS_INVALID_PARAMETER;
    }
...
NTSTATUS Tls13HandshakeMessages::ParseEncryptedExtensions(const TlsHandshakeMessageView &message,
                                                          Tls13EncryptedExtensionsView &encryptedExtensions) noexcept
{
    ...
    {
        status =
            ReadBytes(message.Body, message.BodyLength, &offset, extensionsLength, &encryptedExtensions.Extensions);
    }
    ...
    SIZE_T extensionLength = 0;
    bool found = false;
    status = FindExtension(encryptedExtensions.Extensions, encryptedExtensions.ExtensionsLength, ExtensionAlpn,
                           &extension, &extensionLength, &found);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
    ...
    {
        status =
            ParseAlpnExtension(extension, extensionLength, &encryptedExtensions.Alpn, &encryptedExtensions.AlpnLength);
        if (!NT_SUCCESS(status))
        {
            return status;
        }
    ...
    status = FindExtension(encryptedExtensions.Extensions, encryptedExtensions.ExtensionsLength, ExtensionEarlyData,
                           &extension, &extensionLength, &found);
    if (!NT_SUCCESS(status))
    {
        return status;
...
NTSTATUS Tls13HandshakeMessages::ParseCertificateRequest(const TlsHandshakeMessageView &message,
...
NTSTATUS Tls13HandshakeMessages::ParseCertificate(const TlsHandshakeMessageView &message,
                                                  Tls13CertificateView &certificate) noexcept
{
    ...
    while (certificateOffset < certificate.CertificatesLength)
    {
        ...
        status = ReadBytes(certificate.Certificates, certificate.CertificatesLength, &certificateOffset, certLength,
                           &certData);
        USHORT certExtensionsLength = 0;
        if (NT_SUCCESS(status))
        {
            status = ReadUint16(certificate.Certificates, certificate.CertificatesLength, &certificateOffset,
                                &certExtensionsLength);
        }
        const UCHAR *certExtensions = nullptr;
        if (NT_SUCCESS(status))
        {
            status = ReadBytes(certificate.Certificates, certificate.CertificatesLength, &certificateOffset,
                               certExtensionsLength, &certExtensions);
        }
        if (!NT_SUCCESS(status))
        {
            return status;
...
NTSTATUS Tls13HandshakeMessages::EncodeEmptyCertificate(const UCHAR *requestContext, SIZE_T requestContextLength,
                                                        UCHAR *destination, SIZE_T destinationCapacity,
                                                        SIZE_T *bytesWritten) noexcept
{
    if (bytesWritten != nullptr)
    ...
    if ((requestContext == nullptr && requestContextLength != 0) || requestContextLength > 255)
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    SIZE_T offset = 0;
    NTSTATUS status =
        WriteByte(static_cast<UCHAR>(TlsHandshakeType::Certificate), destination, destinationCapacity, &offset);
    if (NT_SUCCESS(status))
    {
...
NTSTATUS Tls13HandshakeMessages::EncodeCertificate(const UCHAR *requestContext, SIZE_T requestContextLength,
                                                   const UCHAR *certificateList, SIZE_T certificateListLength,
                                                   UCHAR *destination, SIZE_T destinationCapacity,
                                                   SIZE_T *bytesWritten) noexcept
{
    if (bytesWritten != nullptr)
    ...
    if (!IsValidBuffer(requestContext, requestContextLength) ||
        !IsValidBuffer(certificateList, certificateListLength) || requestContextLength > 255 ||
...
NTSTATUS Tls13HandshakeMessages::EncodeCertificateVerify(TlsSignatureScheme signatureScheme, const UCHAR *signature,
                                                         SIZE_T signatureLength, UCHAR *destination,
                                                         SIZE_T destinationCapacity, SIZE_T *bytesWritten) noexcept
{
    if (bytesWritten != nullptr)
    {
        *bytesWritten = 0;
    ...
    SIZE_T offset = 0;
    NTSTATUS status =
        WriteByte(static_cast<UCHAR>(TlsHandshakeType::CertificateVerify), destination, destinationCapacity, &offset);
    if (NT_SUCCESS(status))
    {
        status = WriteUint24(static_cast<ULONG>(bodyLength), destination, destinationCapacity, &offset);
    }
...
NTSTATUS Tls13HandshakeMessages::EncodeEndOfEarlyData(UCHAR *destination, SIZE_T destinationCapacity,
                                                      SIZE_T *bytesWritten) noexcept
{
    if (bytesWritten != nullptr)
    {
    ...
    SIZE_T offset = 0;
    NTSTATUS status =
        WriteByte(static_cast<UCHAR>(TlsHandshakeType::EndOfEarlyData), destination, destinationCapacity, &offset);
    if (NT_SUCCESS(status))
    {
        status = WriteUint24(0, destination, destinationCapacity, &offset);
    }
...
NTSTATUS Tls13HandshakeMessages::ParseCertificateVerify(const TlsHandshakeMessageView &message,
...
NTSTATUS Tls13HandshakeMessages::ParseKeyUpdate(const TlsHandshakeMessageView &message,
...
NTSTATUS Tls13HandshakeMessages::EncodeKeyUpdate(Tls13KeyUpdateRequest request, UCHAR *destination,
                                                 SIZE_T destinationCapacity, SIZE_T *bytesWritten) noexcept
{
    if (bytesWritten != nullptr)
    {
    ...
    if (request != Tls13KeyUpdateRequest::UpdateNotRequested && request != Tls13KeyUpdateRequest::UpdateRequested)
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    SIZE_T offset = 0;
    NTSTATUS status =
        WriteByte(static_cast<UCHAR>(TlsHandshakeType::KeyUpdate), destination, destinationCapacity, &offset);
    if (NT_SUCCESS(status))
    {
...
NTSTATUS Tls13HandshakeMessages::ParseNewSessionTicket(const TlsHandshakeMessageView &message,
...
NTSTATUS Tls13HandshakeMessages::ParseNextNewSessionTicket(const UCHAR *data, SIZE_T dataLength, SIZE_T *offset,
                                                           Tls13NewSessionTicketView &ticket) noexcept
{
    ticket = {};
    ...
    NTSTATUS status = TlsHandshake12::ParseMessage(data + *offset, dataLength - *offset, message);
    ...
    if (message.BytesConsumed == 0 || message.BytesConsumed > dataLength - *offset ||
...
NTSTATUS Tls13HandshakeMessages::EncodeFinished(const TlsContext &context, bool clientFinished,
                                                const UCHAR *transcriptHash, SIZE_T transcriptHashLength,
                                                UCHAR *destination, SIZE_T destinationCapacity,
                                                SIZE_T *bytesWritten) noexcept
{
    ...
    NTSTATUS status = ComputeFinishedData(context, clientFinished, transcriptHash, transcriptHashLength,
                                          verifyData.Get(), verifyDataLength);
    if (!NT_SUCCESS(status))
    {
        return status;
    }
...
NTSTATUS Tls13HandshakeMessages::VerifyFinished(const TlsContext &context, bool clientFinished,
                                                const UCHAR *transcriptHash, SIZE_T transcriptHashLength,
                                                const UCHAR *verifyData, SIZE_T verifyDataLength) noexcept
{
    if (verifyData == nullptr || verifyDataLength == 0 || verifyDataLength != context.Tls13Secrets().SecretLength ||
    ...
    NTSTATUS status = ComputeFinishedData(context, clientFinished, transcriptHash, transcriptHashLength, expected.Get(),
                                          verifyDataLength);
    if (!NT_SUCCESS(status))
    {
        return status;
...
NTSTATUS Tls13HandshakeMessages::BuildCertificateVerifyInput(bool server, const UCHAR *transcriptHash,
                                                             SIZE_T transcriptHashLength, UCHAR *destination,
                                                             SIZE_T destinationCapacity, SIZE_T *bytesWritten) noexcept
{
    if (bytesWritten != nullptr)
    {
    ...
    const char *context = server ? "TLS 1.3, server CertificateVerify" : "TLS 1.3, client CertificateVerify";
