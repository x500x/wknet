...
namespace tls
{
    ...
    class TlsHandshake13 final
    {
    public:
        TlsHandshake13() = delete;

        _Must_inspect_result_
        static crypto::HashAlgorithm HashForCipherSuite(TlsCipherSuite cipherSuite) noexcept;

        _Must_inspect_result_
        static bool IsSupportedCipherSuite(TlsCipherSuite cipherSuite) noexcept;
        ...
        _Must_inspect_result_
        static NTSTATUS ComputePskBinder(
        ...
        _Must_inspect_result_
        static NTSTATUS FindPskBinderTranscriptLength(
        ...
        _Must_inspect_result_
        static NTSTATUS ParseEncryptedExtensions(
            _In_ const TlsHandshakeMessageView& message,
            _Out_ Tls13EncryptedExtensionsView& encryptedExtensions) noexcept;

        _Must_inspect_result_
        static NTSTATUS ParseCertificateRequest(
            _In_ const TlsHandshakeMessageView& message,
            _Out_ Tls13CertificateRequestView& certificateRequest) noexcept;
        ...
        _Must_inspect_result_
        static NTSTATUS BuildCertificateVerifyInput(
            bool server,
            _In_reads_bytes_(transcriptHashLength) const UCHAR* transcriptHash,
            SIZE_T transcriptHashLength,
            _Out_writes_bytes_(destinationCapacity) UCHAR* destination,
