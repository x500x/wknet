...
| 协议 | 已支持能力 | 当前边界 |
|------|------------|----------|
| HTTP/1.1 | `Content-Length`、响应 `Transfer-Encoding` 链（`chunked`/`gzip`/`deflate`/`compress`）、close-delimited 响应、HEAD/101/无 body 状态码、中间 1xx 跳过 | 请求体使用 `Content-Length`；用户设置请求 `Transfer-Encoding` 会被拒绝；chunked 上传和响应 trailer 暴露暂不支持；`br` 仅作为 `Content-Encoding` 支持 |
...
| TLS | TLS 1.2/1.3、ECDHE + AES-GCM 主路径、TLS 1.3 降级保护、证书链、dNSName/iPAddress SAN 和锁定校验 | 不支持 TLS 客户端证书、CBC、ChaCha20-Poly1305 和 OCSP/CRL 撤销检查 |
...
close-delimited HTTP/1.x 响应和 `101 Switching Protocols` 升级响应不会回到普通 HTTP 连接池。同步 HTTP、WebSocket、TLS 和证书验证路径要求在 `PASSIVE_LEVEL` 调用。TLS ALPN 结果必须来自客户端 offer 列表；TLS1.2 选择只有在 TLS1.3 路径取得可验证的版本协商证据后才允许进行，不能把证书错误、ALPN mismatch、网络超时或解密失败当作 TLS1.2-only 结论。
