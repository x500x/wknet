...
| TLS | TLS 1.2/1.3、ECDHE + AES-GCM 主路径、TLS 1.3 降级保护、PSK ticket 绑定、HRR 后 binder 重算、证书链、dNSName/iPAddress SAN 和锁定校验 | 不支持 TLS 客户端证书、CBC、RSA key exchange、ChaCha20-Poly1305、X25519、EdDSA、OCSP/CRL 撤销检查和 IDNA；Name Constraints 明确返回不支持 |
...
| TLS CBC / RSA key exchange / ChaCha20-Poly1305 / X25519 / EdDSA | 暂不在当前 cipher suite、group 或 signature 子集内 |
