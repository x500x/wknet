...
| HTTP/3 prior knowledge | 通过 `SessionConfig.Http3.Mode=Http3ConnectMode::Required` 显式要求 HTTPS origin 的 H3；仅用于明确 prior-knowledge/测试。M6 中默认 `Auto` 仍归一为 `Disabled`，不会透明选择 H3 |
...
| HTTP/3 / QUIC 透明 Auto | 主路径建设中；`Required` 配置用于明确 prior-knowledge/测试，在完整门禁通过前 `Auto` 不启用 |
...
// HTTP/3：默认 Auto，但 M6 内部仍按 Disabled 处理，普通用户行为不变。
// Required 只用于明确知道 HTTPS origin 支持 H3 的 prior-knowledge/测试场景；
// 不支持明文 HTTP、h2c、HTTP proxy、非 HTTP ALPN 或 HTTP/2 priority。
