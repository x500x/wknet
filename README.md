...
| HTTP/3 / QUIC | 默认 `Auto`：首次 HTTPS 请求走 TCP，在已完成证书与策略验证的响应中学习精确 `h3` Alt-Svc，后续请求优先使用 QUIC v1 + HTTP/3；`Disabled` 禁用自动学习/使用，`Required` 用于明确 prior-knowledge |
...
| HTTP/3 非目标 | QUIC v2、0-RTT application data、主动迁移、多路径、ECN、DPLPMTUD、WebTransport、QUIC Datagram 与 WebSocket over HTTP/3 暂不属于主路径 |
...
HTTP/3 `Auto` 的 QUIC probe 只在请求确定未发送时允许 TCP fallback；HEADERS 已提交、body 已消费或响应已开始后不会盲目重放。网络失败按 candidate 与地址族做短期指数退避，协议/证书/ALPN 失败在当前 Alt-Svc 生命周期内禁用该 candidate；取消和本地资源不足不会污染缓存。
...
// HTTP/3：默认 Auto。首次请求使用 TCP；只有已认证 HTTPS 响应中的
// 精确 h3 Alt-Svc 才会被缓存，后续请求才会尝试 QUIC/HTTP3。
// alternative endpoint 只负责网络路由，SNI、证书身份和 :authority
// 始终保持原 origin。代理、明文 HTTP、h2c、WebSocket、显式非 HTTP
// ALPN 和 HTTP/2 priority 不进入 H3。
