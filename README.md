...
| 协议 | 已支持能力 | 当前边界 |
|------|------------|----------|
| HTTP/1.1 | `Content-Length`、显式 chunked 请求体、请求 trailer（chunked 路径）、响应 `Transfer-Encoding` 链（`chunked`/`gzip`/`deflate`/`compress`）、close-delimited 响应、HEAD/101/无 body 状态码、中间 1xx 跳过、chunked trailer 语法/禁止字段校验与只读 API 暴露、`206`/`Content-Range` 只读解析、RFC 3986 相对 redirect 解析、CONNECT 方法构建 | 用户设置请求 `Transfer-Encoding`/`TE` 会被拒绝；request trailer 仅限 chunked 路径；不提供入站 request parser/server role；`HttpsClient` 支持显式 HTTP/1.1 CONNECT 代理隧道，但高层 Session 级全局代理配置未暴露；TRACE 不支持；`Range`/条件请求作为普通 header 透传；`Accept-Encoding` 不承诺完整 qvalue/content negotiation；`br` 仅作为 `Content-Encoding` 支持 |
| HTTP/2 | TLS ALPN、h2c prior knowledge / Upgrade、SETTINGS（含 `ENABLE_CONNECT_PROTOCOL`）、HEADERS/CONTINUATION、DATA、PING、GOAWAY、WINDOW_UPDATE、HPACK、header block 语义校验、HPACK header-list/table-size 限制、活动 stream 表与 `BeginRequest`/`ReceiveResponse(streamId)` 两阶段接口、RFC 8441 extended CONNECT 低层 DATA tunnel | 高层 `khttp`/`HttpsClient` 尚未做 h2 连接池级复用；高层 `kws` 不自动选择 RFC 8441；server push、priority 不作为公共能力；收到禁用的 `PUSH_PROMISE` 视为协议错误；缺失 SETTINGS ACK 会以 `SETTINGS_TIMEOUT` 关闭 |
| WebSocket | ws/wss 握手（Accept 常量时间校验）、自定义 opening handshake headers（拒绝库受控头和非法文本）、文本/二进制发送、空消息、**分片发送（`kws::SendContinuation`）与接收分片回调（`ReceiveOptions.OnMessage`）**、控制帧校验、自动 Pong、Ping/Pong/CloseEx、selected subprotocol 查询、跨片 UTF-8 校验、默认聚合完整消息 | 高层 `kws` 主路径是 HTTP/1.1 Upgrade；扩展协商不支持（拒绝 `Sec-WebSocket-Extensions`）；RFC 8441 仅有低层 HTTP/2 tunnel primitive；主动 close 发送 close frame 后等待 peer close（3s 超时），收到 peer close 时 echo 后关闭 |
| TLS | TLS 1.2/1.3、TLS 1.3 全部标准 cipher suite、TLS 1.2 ECDHE/DHE/RSA key exchange、AES-GCM/AES-CBC/ChaCha20-Poly1305、X25519/X448/NIST P curves/FFDHE、RSA-PSS/RSA-PKCS1/ECDSA/Ed25519 signature scheme、SNI、ALPN、PSK/session ticket、0-RTT 显式 opt-in、KeyUpdate（被动）、record padding、客户端证书（mTLS）、OCSP stapling 解析、证书链重排和校验、Name Constraints、certificatePolicies、IDNA、OCSP/CRL 撤销缓存、SPKI pin。ChaCha20-Poly1305/AES-CCM/X25519/X448/FFDHE/Ed25519 验签为内核内软件实现 | 默认策略不启用 TLS 1.2 RSA key exchange、CBC、renegotiation 或 SHA-1 签名；这些只在 `TlsSecurityProfile::CompatibilityExplicit` 且相应开关显式开启时可用。Ed448 验签未实现且不主动宣称。库层不硬编码系统 CA，不把 WinHTTP/WinINet/SChannel 作为内核主路径；在线撤销获取由调用侧提供有界输入或缓存（required-but-absent 时 fail-closed） |

| 未实现 optional 能力 | 当前处理 |
|----------------------|----------|
| WebSocket extensions（如 permessage-deflate） | 作为非目标，服务端返回未请求扩展时拒绝 |
| 高层 WebSocket 自动选择 RFC 8441 | 延期；低层 HTTP/2 已提供 extended CONNECT tunnel primitive，当前 `kws` 主路径仍是 HTTP/1.1 Upgrade |
| 高层 Session 全局代理配置 / TRACE | Session 级代理配置未暴露；低层 `HttpsClient` 可显式建立 HTTP/1.1 CONNECT 代理隧道；TRACE 不支持 |
| HTTP 入站 request parser / server role | 非目标，当前项目定位为客户端协议栈 |
| 高层 HTTP/2 连接池级复用 / server push / priority | 低层已有多活动流基础；高层连接池复用未接入；禁用 push，收到非法 `PUSH_PROMISE` 视为协议错误；priority 不作为公共调度能力 |
