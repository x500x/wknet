...
| 协议 | 已支持能力 | 当前边界 |
|------|------------|----------|
| HTTP/1.1 | `Content-Length`、响应 `Transfer-Encoding` 链（`chunked`/`gzip`/`deflate`/`compress`）、close-delimited 响应、HEAD/101/无 body 状态码、中间 1xx 跳过、chunked trailer 语法和禁止字段校验、RFC 3986 相对 redirect 解析 | 请求体使用 `Content-Length`；用户设置请求 `Transfer-Encoding` 会被拒绝；chunked 上传和响应 trailer 暴露暂不支持；`br` 仅作为 `Content-Encoding` 支持 |
| HTTP/2 | TLS ALPN、h2c prior knowledge / Upgrade、SETTINGS、HEADERS/CONTINUATION、DATA、PING、GOAWAY、WINDOW_UPDATE、HPACK、header block 语义校验、HPACK header-list/table-size 限制 | 不支持 server push、priority 和复杂多流调度；收到禁用的 `PUSH_PROMISE` 视为协议错误；响应必须以 `END_STREAM`、`RST_STREAM` 或 `GOAWAY` 结束 |
| WebSocket | ws/wss 握手、文本/二进制发送、空消息、控制帧校验、Ping/Pong/Close、默认接收完整消息 | 不支持扩展协商和接收分片回调；默认 API 聚合为完整消息 |
| TLS | TLS 1.2/1.3、ECDHE + AES-GCM 主路径、TLS 1.3 降级保护、PSK ticket 绑定、HRR 后 binder 重算、证书链、dNSName/iPAddress SAN 和锁定校验 | 不支持 TLS 客户端证书、CBC、ChaCha20-Poly1305、OCSP/CRL 撤销检查和 IDNA；Name Constraints 明确返回不支持 |
...
| HTTP 响应 trailer 暴露 | parser 校验并消费 trailer，但 API 暂不暴露 |
