...
| 协议 | 已支持能力 | 当前边界 |
|------|------------|----------|
| HTTP/1.1 | `Content-Length`、显式 chunked 请求体、请求 trailer（chunked 路径）、响应 `Transfer-Encoding` 链（`chunked`/`gzip`/`deflate`/`compress`）、close-delimited 响应、HEAD/101/无 body 状态码、中间 1xx 跳过、chunked trailer 语法/禁止字段校验与只读 API 暴露、`206`/`Content-Range` 只读解析、RFC 3986 相对 redirect 解析、CONNECT 方法构建；高层 Session 可显式配置 HTTPS 代理 CONNECT 隧道 | 用户设置请求 `Transfer-Encoding`/`TE` 会被拒绝；request trailer 仅限 chunked 路径；不提供入站 request parser/server role；明文 HTTP over proxy 当前显式拒绝；TRACE 不支持；`Range`/条件请求作为普通 header 透传；`Accept-Encoding` 不承诺完整 qvalue/content negotiation；`br` 仅作为 `Content-Encoding` 支持 |
| HTTP/2 | TLS ALPN、h2c prior knowledge / Upgrade、SETTINGS（含 `ENABLE_CONNECT_PROTOCOL`）、HEADERS/CONTINUATION、DATA、PING/主动 `SendPing`、GOAWAY、WINDOW_UPDATE、HPACK、header block 语义校验、HPACK header-list/table-size 限制、活动 stream 表与 `BeginRequest`/`ReceiveResponse(streamId)` 两阶段接口、RFC 8441 extended CONNECT DATA tunnel；高层 `khttp` 连接池已接入 HTTP/2 多活动流复用 | server push、priority 不作为公共能力；收到禁用的 `PUSH_PROMISE` 视为协议错误；高层 `khttp` 不暴露 h2c；缺失 SETTINGS ACK 会以 `SETTINGS_TIMEOUT` 关闭 |
...
| 明文 HTTP over proxy / TRACE | HTTPS 代理 CONNECT 已支持；明文 HTTP over proxy 当前显式拒绝，TRACE 不支持 |
...
// HTTPS 代理 CONNECT 隧道（显式配置；明文 HTTP over proxy 当前拒绝）
