...
- **🎯 两层 API**：提供产品面 API（`wknet::http` / `wknet::websocket` / `wknet::crypto` / `wknet::codec`）
...
│       │   ├── Detail.h             # 内部桥接接口
│       │   └── Test.h               # 测试辅助
│       ├── websocket/                     # 高层 WebSocket API（wknet::websocket）
...
| 产品 API（http/websocket/crypto/codec） | [High-Level API](https://github.com/x500x/khttp/wiki/High-Level-API) |
| 内部层（session/http1/http2/tls/net） | [Low-Level API](https://github.com/x500x/khttp/wiki/Low-Level-API) |
...
| `khttp_tests.cpp` | 产品 HTTP API 测试 |
