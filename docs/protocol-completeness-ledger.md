...
| 条目 | RFC 级别 | 状态 | 代码入口 | 测试入口 | 备注 |
|------|----------|------|----------|----------|------|
| HTTP/1.1 Upgrade opening handshake | MUST | 已实现/已验证 | `ws/WebSocketFrame.cpp`、`session/WsHandshake.cpp`、`session/WsConnect.cpp` | `tests/websocket_frame_tests.cpp`、`tests/websocket_client_tests.cpp` | `Sec-WebSocket-Accept` 常量时间比对。 |
| 自定义 opening headers | MAY | 已实现/已验证 | `session/WsHandshake.cpp`、`http_api/WebSocket.cpp` | `tests/websocket_client_tests.cpp` | 库受控头拒绝。 |
| 客户端帧 masking | MUST | 已实现/已验证 | `WebSocketFrame.cpp` | `tests/websocket_frame_tests.cpp` | HTTP/2 RFC 8441 路径按规范无 mask。 |
| 服务端 masked frame | MUST reject | 已实现/已验证 | `ws/WebSocketFrame.cpp`、`session/WsReceive.cpp` | `tests/websocket_frame_tests.cpp` | 关闭码 1002。 |
| 分片发送与默认聚合接收 | MUST | 已实现/已验证 | `session/WsSendData.cpp`、`session/WsReceive.cpp` | `tests/websocket_client_tests.cpp` | 文本跨片 UTF-8 校验。 |
| wire fragment 回调 | SHOULD for streaming clients | 已实现/已验证 | `session/WsReceive.cpp`、`http_api/WebSocket.cpp` | `tests/websocket_client_tests.cpp` | `ReceiveOptions.DeliverFragments` 显式开启；默认仍聚合完整消息。 |
| control frame 自动 Pong 与洪泛上限 | SHOULD/安全边界 | 已实现/已验证 | `session/WsControl.cpp`、`session/WsReceive.cpp` | `tests/websocket_client_tests.cpp` | 单次接收控制帧 ≤100。 |
| close handshake | MUST | 已实现/已验证 | `session/WsClose.cpp`、`session/WsControl.cpp` | `tests/websocket_client_tests.cpp` | 主动/被动 close 已覆盖。 |
| RFC 8441 WebSocket over HTTP/2 | MAY | 自动协商/已验证 | `http2/Http2Connection.cpp`、`session/WsConnect.cpp` | `tests/http2_client_tests.cpp`、`tests/websocket_client_tests.cpp` | `wss` 默认自动选择，`Http11Only` 可关闭。 |
| permessage-deflate | MAY | 显式 opt-in 已实现/已验证 | `ws/WebSocketFrame.cpp`、`ws/WebSocketDeflate.cpp`、`session/WsConnect.cpp` | `tests/websocket_frame_tests.cpp`、`tests/websocket_client_tests.cpp` | 默认关闭；未请求扩展、未知扩展、重复/非法参数拒绝；解压受消息大小、输出容量和膨胀比限制。 |
| opening-handshake redirect/401/407 跟随 | MAY | 明确非目标/安全拒绝 | `session/WsHandshake.cpp`、`session/WsConnect.cpp` | `tests/websocket_client_tests.cpp` | 3xx/401/407 返回 `STATUS_NOT_SUPPORTED`；若未来实现必须显式 opt-in 且沿用 HTTP 安全重定向规则。 |
