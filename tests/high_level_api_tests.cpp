...
namespace
{
    ...
    void TestHighLevelSampleRetriesTransientHttpFailures()
    {
        ...
        Expect(results.RemoteHttpsIpv4Async.Status == STATUS_SUCCESS, "remote IPv4 async result succeeds after retry");
        Expect(results.RemoteHttpsIpv6.Status == STATUS_SUCCESS, "remote IPv6 result succeeds after retry");
        Expect(results.RemoteHttpsIpv6Async.Status == STATUS_SUCCESS, "remote IPv6 async result succeeds after retry");
        Expect(transport.CallCount == 6, "sample retries only transient failures before continuing sync and async matrix");
    ...
    void TestHighLevelSampleRunners()
    {
        ...
        Expect(results.HttpPostAsync.Status == STATUS_SUCCESS, "main samples include async HTTP POST");
        Expect(results.HttpPut.Status == STATUS_SUCCESS, "main samples include HTTP PUT");
        Expect(results.HttpPutAsync.Status == STATUS_SUCCESS, "main samples include async HTTP PUT");
        Expect(results.HttpPatch.Status == STATUS_SUCCESS, "main samples include HTTP PATCH");
        Expect(results.HttpPatchAsync.Status == STATUS_SUCCESS, "main samples include async HTTP PATCH");
        Expect(results.HttpDelete.Status == STATUS_SUCCESS, "main samples include HTTP DELETE");
        Expect(results.HttpDeleteAsync.Status == STATUS_SUCCESS, "main samples include async HTTP DELETE");
        Expect(results.HttpHead.Status == STATUS_SUCCESS, "main samples include HTTP HEAD");
        Expect(results.HttpHeadAsync.Status == STATUS_SUCCESS, "main samples include async HTTP HEAD");
        Expect(results.HttpOptions.Status == STATUS_SUCCESS, "main samples include HTTP OPTIONS");
        Expect(results.HttpOptionsAsync.Status == STATUS_SUCCESS, "main samples include async HTTP OPTIONS");
        Expect(results.HttpsTlsOptions.Status == STATUS_SUCCESS, "main samples include HTTPS TLS options");
        Expect(results.HttpsTlsOptionsAsync.Status == STATUS_SUCCESS, "main samples include async HTTPS TLS options");
        Expect(results.HttpsPost.Status == STATUS_SUCCESS, "main samples include HTTPS POST");
        Expect(results.HttpsPostAsync.Status == STATUS_SUCCESS, "main samples include async HTTPS POST");
        Expect(results.HttpsPut.Status == STATUS_SUCCESS, "main samples include HTTPS PUT");
        Expect(results.HttpsPutAsync.Status == STATUS_SUCCESS, "main samples include async HTTPS PUT");
        Expect(results.HttpsPatch.Status == STATUS_SUCCESS, "main samples include HTTPS PATCH");
        Expect(results.HttpsPatchAsync.Status == STATUS_SUCCESS, "main samples include async HTTPS PATCH");
        Expect(results.HttpsDelete.Status == STATUS_SUCCESS, "main samples include HTTPS DELETE");
        Expect(results.HttpsDeleteAsync.Status == STATUS_SUCCESS, "main samples include async HTTPS DELETE");
        Expect(results.HttpsHead.Status == STATUS_SUCCESS, "main samples include HTTPS HEAD");
        Expect(results.HttpsHeadAsync.Status == STATUS_SUCCESS, "main samples include async HTTPS HEAD");
        Expect(results.HttpsOptions.Status == STATUS_SUCCESS, "main samples include HTTPS OPTIONS");
        Expect(results.HttpsOptionsAsync.Status == STATUS_SUCCESS, "main samples include async HTTPS OPTIONS");
        Expect(results.Http2Alpn.Status == STATUS_SUCCESS, "main samples include HTTP/2 ALPN option");
        Expect(results.Http2AlpnAsync.Status == STATUS_SUCCESS, "main samples include async HTTP/2 ALPN option");
        ...
        Expect(results.RemoteHttpsIpv4Async.Status == STATUS_SUCCESS, "main samples include async forced remote IPv4 HTTPS");
        Expect(results.RemoteHttpsIpv6.Status == STATUS_SUCCESS, "main samples include forced remote IPv6 HTTPS");
        Expect(results.RemoteHttpsIpv6Async.Status == STATUS_SUCCESS, "main samples include async forced remote IPv6 HTTPS");
        ...
        Expect(transport.H2AlpnCount == 2, "main samples request h2 ALPN for sync and async HTTP/2");
        ...
        Expect(transport.Ipv6Count == 2, "main samples send sync and async forced remote IPv6 HTTPS requests");
        Expect(transport.VerifiedHttpsForceNewCount == 20, "main verified HTTPS samples force fresh TLS connections for sync and async requests");
        ...
        Expect(results.HttpGet.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP GET");
        Expect(results.HttpGetAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP GET");
        Expect(results.HttpPost.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP POST");
        Expect(results.HttpPostAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP POST");
        Expect(results.HttpPut.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP PUT");
        Expect(results.HttpPutAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP PUT");
        Expect(results.HttpPatch.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP PATCH");
        Expect(results.HttpPatchAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP PATCH");
        Expect(results.HttpDelete.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP DELETE");
        Expect(results.HttpDeleteAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP DELETE");
        Expect(results.HttpHead.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP HEAD");
        Expect(results.HttpHeadAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP HEAD");
        Expect(results.HttpOptions.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP OPTIONS");
        Expect(results.HttpOptionsAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP OPTIONS");
        Expect(results.HttpsTlsOptions.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS GET");
        Expect(results.HttpsTlsOptionsAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS GET");
        Expect(results.HttpsPost.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS POST");
        Expect(results.HttpsPostAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS POST");
        Expect(results.HttpsPut.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS PUT");
        Expect(results.HttpsPutAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS PUT");
        Expect(results.HttpsPatch.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS PATCH");
        Expect(results.HttpsPatchAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS PATCH");
        Expect(results.HttpsDelete.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS DELETE");
        Expect(results.HttpsDeleteAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS DELETE");
        Expect(results.HttpsHead.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS HEAD");
        Expect(results.HttpsHeadAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS HEAD");
        Expect(results.HttpsOptions.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS OPTIONS");
        Expect(results.HttpsOptionsAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS OPTIONS");
        Expect(results.Http2Alpn.Status == STATUS_SUCCESS, "test-driver matrix includes HTTP/2 ALPN");
        Expect(results.Http2AlpnAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTP/2 ALPN");
        Expect(results.HttpsNoVerify.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS no-verify");
        Expect(results.HttpsNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS no-verify");
        Expect(results.HttpsPostNoVerify.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS POST no-verify");
        Expect(results.HttpsPostNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS POST no-verify");
        Expect(results.HttpsPutNoVerify.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS PUT no-verify");
        Expect(results.HttpsPutNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS PUT no-verify");
        Expect(results.HttpsPatchNoVerify.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS PATCH no-verify");
        Expect(results.HttpsPatchNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS PATCH no-verify");
        Expect(results.HttpsDeleteNoVerify.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS DELETE no-verify");
        Expect(results.HttpsDeleteNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS DELETE no-verify");
        Expect(results.HttpsHeadNoVerify.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS HEAD no-verify");
        Expect(results.HttpsHeadNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS HEAD no-verify");
        Expect(results.HttpsOptionsNoVerify.Status == STATUS_SUCCESS, "test-driver matrix includes HTTPS OPTIONS no-verify");
        Expect(results.HttpsOptionsNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async HTTPS OPTIONS no-verify");
        ...
        Expect(results.WebSocketEchoNoVerifyAsync.Status == STATUS_SUCCESS, "test-driver matrix includes async WebSocket no-verify");
        ...
        Expect(results.RemoteHttpsIpv4Async.Status == STATUS_SUCCESS, "remote nghttp2 address-family samples include async IPv4");
        Expect(results.RemoteHttpsIpv6.Status == STATUS_SUCCESS, "remote nghttp2 address-family samples include IPv6");
        Expect(results.RemoteHttpsIpv6Async.Status == STATUS_SUCCESS, "remote nghttp2 address-family samples include async IPv6");
        ...
        Expect(transport.NoVerifyCount == 14, "test-driver matrix exercises sync and async no-verify HTTPS scenarios");
        Expect(transport.Ipv4Count >= 1, "test-driver matrix sends forced IPv4 HTTPS requests");
        Expect(transport.Ipv6Count == 2, "test-driver matrix sends sync and async forced remote IPv6 HTTPS requests");
        Expect(transport.H2AlpnCount == 2, "test-driver matrix exercises sync and async HTTP/2 ALPN");
        Expect(transport.VerifiedHttpsForceNewCount == 20, "test-driver verified HTTPS matrix forces fresh TLS connections for sync and async requests");
        Expect(transport.VerifiedHttpsReuseCount == 0, "test-driver verified HTTPS matrix avoids idle TLS reuse");
        Expect(capture.ConnectCount == 4 && capture.SendCount == 4 && capture.ReceiveCount == 8, "test-driver matrix skips websocket banners before sync and async verify/no-verify text echoes");
