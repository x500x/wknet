...
namespace
{
    ...
    void CaptureTrace(void*, wknet::TraceLevel, ULONG, const char* message) noexcept
    {
        ...
        if (strstr(message, "payload-secret") != nullptr || strstr(message, "token-secret") != nullptr ||
    ...
    bool AddressEquals(const SOCKADDR_STORAGE& left, const SOCKADDR_STORAGE& right) noexcept
    {
        ...
        return left.ss_family == right.ss_family && RtlCompareMemory(&left, &right, length) == length;
    }
    ...
    void TestLifecycleAndSynchronousReceive() noexcept
    {
        ...
        SIZE_T sent = 99;
        Expect(wknet::net::WskDatagramSocketSend(fixture.Socket, payload, sizeof(payload) - 1, &sent) ==
        ...
        Expect(NT_SUCCESS(wknet::net::WskDatagramSocketBind(fixture.Socket, reinterpret_cast<const SOCKADDR*>(&local))),
               "IPv4 socket binds");
        Expect(wknet::net::WskDatagramSocketBind(fixture.Socket, reinterpret_cast<const SOCKADDR*>(&local)) ==
                   STATUS_INVALID_DEVICE_STATE,
        ...
        Expect(
            NT_SUCCESS(wknet::net::WskDatagramSocketStartReceive(fixture.Socket, receiveBuffer, sizeof(receiveBuffer))),
            "synchronous provider completion is accepted");
        Expect(wknet::net::WskDatagramSocketStartReceive(fixture.Socket, receiveBuffer, sizeof(receiveBuffer)) ==
                   STATUS_DEVICE_BUSY,
               "receive remains owned until CompleteReceive consumes it");

        wknet::net::WskDatagramReceiveResult result = {};
        Expect(NT_SUCCESS(wknet::net::WskDatagramSocketCompleteReceive(fixture.Socket, 1000, &result)),
        ...
        Expect(memcmp(receiveBuffer, packet, sizeof(packet)) == 0, "provider writes caller NonPaged receive buffer");
        ...
        Expect(NT_SUCCESS(
                   wknet::net::WskDatagramSocketConnectPeer(fixture.Socket, reinterpret_cast<const SOCKADDR*>(&peer))),
        ...
        Expect(NT_SUCCESS(wknet::net::WskDatagramSocketSend(fixture.Socket, payload, sizeof(payload) - 1, &sent)),
        ...
        wknet::net::test::SetNextSendResult(STATUS_SUCCESS, sizeof(payload) - 1);
        Expect(NT_SUCCESS(wknet::net::WskDatagramSocketSendTo(fixture.Socket, payload, sizeof(payload) - 1,
                                                              reinterpret_cast<const SOCKADDR*>(&alternate), &sent)),
    ...
    void TestIpv6AndFamilyValidation() noexcept
    {
        ...
        Expect(wknet::net::WskDatagramSocketBind(fixture.Socket, reinterpret_cast<const SOCKADDR*>(&ipv4)) ==
    ...
    void TestDispatchCompletionToPassiveConsumer() noexcept
    {
        ...
        (void)wknet::net::WskDatagramSocketBind(fixture.Socket, reinterpret_cast<const SOCKADDR*>(&local));
        ...
        std::thread consumer(
            [&]() noexcept
            {
                ...
                consumerStatus = wknet::net::WskDatagramSocketCompleteReceive(fixture.Socket, 5000, &result);
            });
        ...
        Expect(wknet::net::test::CompletePendingReceive(), "provider completes pending receive at simulated DISPATCH");
        consumer.join();
        ...
        const wknet::net::test::WskDatagramProviderStatistics stats = wknet::net::test::GetProviderStatistics();
    ...
    void TestCancelTimeoutAndLateCloseCompletion() noexcept
    {
        ...
        wknet::net::WskDatagramReceiveResult result = {};
        Expect(wknet::net::WskDatagramSocketCompleteReceive(fixture.Socket, 1, &result) == STATUS_IO_TIMEOUT,
        ...
        Expect(wknet::net::test::CompleteCancelledReceiveLate(), "cancelled provider request can complete late");
        Expect(wknet::net::WskDatagramSocketCompleteReceive(fixture.Socket, 1000, &result) == STATUS_CANCELLED,
        ...
        Expect(wknet::net::test::CompleteCancelledReceiveLate(), "Close path accepts delayed cancellation completion");
        closer.join();
    ...
    void TestFailureInjectionAndReceiveValidation() noexcept
    {
        ...
        Expect(wknet::net::WskDatagramSocketCreate(client, AF_INET, &failedSocket) == STATUS_INSUFFICIENT_RESOURCES,
        ...
        constexpr wknet::rtl::ProtocolAllocationSite receiveSites[] = {
            wknet::rtl::ProtocolAllocationSite::DatagramReceiveMdl,
            wknet::rtl::ProtocolAllocationSite::DatagramReceiveIrp,
            wknet::rtl::ProtocolAllocationSite::DatagramReceiveDescriptor,
            wknet::rtl::ProtocolAllocationSite::DatagramCompletionRecord};
        for (const auto site : receiveSites)
        {
            wknet::rtl::ProtocolFailureInjectionSetFailOnNth(site, 1);
            Expect(wknet::net::WskDatagramSocketStartReceive(fixture.Socket, receiveBuffer, sizeof(receiveBuffer)) ==
                       STATUS_INSUFFICIENT_RESOURCES,
                   "stable receive allocation site propagates failure");
            Expect(wknet::rtl::ProtocolFailureInjectionHitCount(site) == 1,
                   "stable receive allocation site is hit exactly once");
            Expect(wknet::rtl::ProtocolFailureInjectionLiveCount(site) == 0,
                   "failed receive allocation site has no live resource");
            Expect(wknet::net::test::GetProviderStatistics().BufferReferences == 0,
                   "receive failpoint does not retain caller buffer");
        }
        ...
        wknet::net::WskDatagramReceiveResult result = {};
        Expect(wknet::net::WskDatagramSocketCompleteReceive(fixture.Socket, 1000, &result) ==
        ...
        SIZE_T sent = 0;
        Expect(wknet::net::WskDatagramSocketSendTo(fixture.Socket, payload, sizeof(payload) - 1,
                                                   reinterpret_cast<const SOCKADDR*>(&local),
                                                   &sent) == STATUS_CONNECTION_RESET,
               "send provider failure is propagated");

        wknet::rtl::ProtocolFailureInjectionSetFailOnNth(wknet::rtl::ProtocolAllocationSite::DatagramSendOperation, 1);
        Expect(wknet::net::WskDatagramSocketSendTo(fixture.Socket, payload, sizeof(payload) - 1,
                                                   reinterpret_cast<const SOCKADDR*>(&local),
                                                   &sent) == STATUS_INSUFFICIENT_RESOURCES,
               "send operation failpoint is propagated");
        Expect(wknet::rtl::ProtocolFailureInjectionLiveCount(
                   wknet::rtl::ProtocolAllocationSite::DatagramSendOperation) == 0,
               "failed send operation has no live resource");

        Expect(wknet::rtl::ProtocolFailureInjectionLiveCount(
                   wknet::rtl::ProtocolAllocationSite::DatagramSocketObject) == 1,
               "fixture owns one tracked socket");
        fixture.Cleanup();
        Expect(wknet::rtl::ProtocolFailureInjectionLiveCount(
                   wknet::rtl::ProtocolAllocationSite::DatagramSocketObject) == 0,
               "socket destroy returns tracked object count to zero");
        Expect(wknet::rtl::ProtocolFailureInjectionTotalLiveCount() == 0,
               "datagram failure paths release every tracked resource");
    }
    ...
    void TestTraceContract() noexcept
    {
        ...
        const char payload[] = "payload-secret";
        SIZE_T sent = 0;
        (void)wknet::net::WskDatagramSocketSendTo(fixture.Socket, payload, sizeof(payload) - 1,
                                                  reinterpret_cast<const SOCKADDR*>(&local), &sent);
        ...
        UCHAR receiveBuffer[8] = {};
        (void)wknet::net::WskDatagramSocketStartReceive(fixture.Socket, receiveBuffer, sizeof(receiveBuffer));
        wknet::net::WskDatagramReceiveResult result = {};
        (void)wknet::net::WskDatagramSocketCompleteReceive(fixture.Socket, 1000, &result);
        fixture.Cleanup();

        Expect(strstr(g_trace, "net.datagram.opened") != nullptr, "opened lifecycle event is emitted");
        Expect(strstr(g_trace, "net.datagram.close_started") != nullptr, "close-started lifecycle event is emitted");
        Expect(strstr(g_trace, "net.datagram.close_completed") != nullptr,
               "close-completed lifecycle event is emitted");
        Expect(strstr(g_trace, "net.datagram.send.failed") != nullptr, "send failure event is emitted");
        Expect(strstr(g_trace, "net.datagram.receive.failed") != nullptr, "receive failure event is emitted");
        Expect(strstr(g_trace, "[conn=8128]") != nullptr, "datagram events carry ConnectionId correlation");
