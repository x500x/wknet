...
{
    Expect(wknet::http3::Http3ClassifyFrameType(wknet::http3::Http3FrameTypeData) == wknet::http3::Http3FrameKind::Data,
           "DATA classified");
...
void TestSingleValuePayload() noexcept
{
    ...
    NTSTATUS status = wknet::http3::Http3ConsumeSingleValuePayload(&parser, first, sizeof(first), &consumed, &complete,
                                                                   &value, &errorCode);
    Expect(NT_SUCCESS(status) && !complete, "single-value payload waits for fragmented varint");

    static const UCHAR second[] = {0x40};
    status = wknet::http3::Http3ConsumeSingleValuePayload(&parser, second, sizeof(second), &consumed, &complete, &value,
                                                          &errorCode);
...
void TestSettings() noexcept
{
    ...
    while (offset < sizeof(payload))
    {
        ...
        if (settingReady)
        {
            if (settingCount == 0)
            {
                Expect(setting.Identifier == wknet::http3::Http3SettingQpackMaxTableCapacity && setting.Value == 32 &&
    ...
    status =
        wknet::http3::Http3ConsumeSettings(&duplicate, duplicatePayload + consumed, sizeof(duplicatePayload) - consumed,
                                           &consumed, &settingReady, &setting, &complete, &errorCode);
...
{
    ULONGLONG errorCode = 0;
    Expect(NT_SUCCESS(wknet::http3::Http3ValidateFramePlacement(wknet::http3::Http3FrameKind::Data,
                                                                wknet::http3::Http3StreamKind::Request,
    ...
    Expect(NT_SUCCESS(wknet::http3::Http3ValidateFramePlacement(wknet::http3::Http3FrameKind::Unknown,
                                                                wknet::http3::Http3StreamKind::Request,
                                                                wknet::http3::Http3EndpointRole::Server, &errorCode)),
           "unknown frame ignored on request stream");
}

wknet::qpack::QpackStringView View(const UCHAR *data, SIZE_T length) noexcept
{
    return {data, length};
}
...
void TestRequestFields() noexcept
{
    ...
    status = wknet::http3::Http3BuildRequestFields(options, output, sizeof(output) / sizeof(output[0]), &outputCount,
                                                   &errorCode);
...
void TestResponseSemantics() noexcept
{
    ...
    Expect(NT_SUCCESS(wknet::http3::Http3ProcessResponseFields(&state, trailers, sizeof(trailers) / sizeof(trailers[0]),
                                                               true, &errorCode)),
    ...
    Expect(NT_SUCCESS(wknet::http3::Http3ProcessResponseFields(
           ...
           "HEAD final headers are accepted");
    ...
    wknet::qpack::QpackFieldView invalid204[] = {
        ...
        {View(contentLengthName, sizeof(contentLengthName) - 1), View(length3, sizeof(length3) - 1), false}};
    Expect(wknet::http3::Http3ProcessResponseFields(&noContent, invalid204, sizeof(invalid204) / sizeof(invalid204[0]),
                                                    false, &errorCode) == STATUS_INVALID_NETWORK_RESPONSE,
           "204 response rejects a nonzero Content-Length");
...
void OnHttp3Headers(void *context, ULONGLONG streamId, const wknet::qpack::QpackFieldView *fields, SIZE_T fieldCount,
...
SIZE_T AppendTestVarInt(ULONGLONG value, UCHAR *output, SIZE_T capacity, SIZE_T offset) noexcept
{
    ...
    if (encodedLength == 0 || offset > capacity ||
        !NT_SUCCESS(wknet::quic::QuicEncodeVarInt(value, encodedLength, output + offset, capacity - offset, &written)))
    {
        return capacity + 1;
    }
...
NTSTATUS InjectStream(wknet::quic::QuicConnection *connection, ULONGLONG streamId, ULONGLONG offset, const UCHAR *data,
...
void TestConnectionControlQpackAndRequest() noexcept
{
    ...
    Expect(NT_SUCCESS(peerEncoder.EncodeFieldSection(requestStreamId, responseFields,
                                                     sizeof(responseFields) / sizeof(responseFields[0]), fieldSection,
                                                     sizeof(fieldSection), &fieldSectionLength)),
           "blocked response field section encodes");

    UCHAR headersFrame[160] = {};
    SIZE_T headersLength = AppendTestVarInt(wknet::http3::Http3FrameTypeHeaders, headersFrame, sizeof(headersFrame), 0);
    ...
    Expect(NT_SUCCESS(peerEncoder.EncodeFieldSection(openReset.StreamId, responseFields,
                                                     sizeof(responseFields) / sizeof(responseFields[0]), cancelSection,
                                                     sizeof(cancelSection), &cancelSectionLength)),
    ...
    Expect(NT_SUCCESS(InjectStream(connection, requestStreamId, headersLength, dataFrame, sizeof(dataFrame), true)) &&
...
{
    {
        ...
        wknet::quic::QuicConnection *connection = nullptr;
        ULONGLONG requestStreamId = 0;
        Expect(CreateHttp3Harness(&capture, &http3, &connection, &requestStreamId), "missing SETTINGS harness creates");
        if (http3 != nullptr && connection != nullptr)
        {
