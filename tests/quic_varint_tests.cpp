...
namespace
{
    ...
    void TestRoundTrip(ULONGLONG value, SIZE_T encodedLength) noexcept
    {
        UCHAR encoded[8] = {};
        SIZE_T written = 0;
        Expect(NT_SUCCESS(wknet::quic::QuicEncodeVarInt(
            value, encodedLength, encoded, sizeof(encoded), &written)),
            "varint encodes");
        Expect(written == encodedLength, "varint uses requested legal width");
        ULONGLONG decoded = 0;
        SIZE_T consumed = 0;
        Expect(NT_SUCCESS(wknet::quic::QuicDecodeVarInt(
            encoded, written, &decoded, &consumed)),
            "varint decodes");
...
int main()
{
    ...
    UCHAR encoded[8] = {};
    SIZE_T written = 0;
    Expect(wknet::quic::QuicEncodeVarInt(
        wknet::quic::QuicVarIntMaximum + 1, 0, encoded, sizeof(encoded), &written) ==
        STATUS_INTEGER_OVERFLOW,
        "62-bit overflow is rejected");
    Expect(wknet::quic::QuicEncodeVarInt(64, 1, encoded, sizeof(encoded), &written) ==
        STATUS_INTEGER_OVERFLOW,
        "value that does not fit requested width is rejected");
    Expect(wknet::quic::QuicEncodeVarInt(64, 2, encoded, 1, &written) ==
    ...
    ULONGLONG value = 0;
    SIZE_T consumed = 0;
    Expect(wknet::quic::QuicDecodeVarInt(
        truncated, sizeof(truncated), &value, &consumed) == STATUS_BUFFER_TOO_SMALL,
