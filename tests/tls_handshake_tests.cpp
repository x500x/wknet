...
using wknet::tls::Tls13ExtensionView;
...
bool FindExtension(const UCHAR *clientHello, SIZE_T clientHelloLength, USHORT extensionType, const UCHAR **extension,
...
bool ClientHelloOffersNamedGroup(const UCHAR *clientHello, SIZE_T clientHelloLength, TlsNamedGroup expected)
{
    const UCHAR *extension = nullptr;
    SIZE_T extensionLength = 0;
    if (!FindExtension(clientHello, clientHelloLength, ExtensionSupportedGroups, &extension, &extensionLength) ||
...
bool ClientHelloOffersCipherSuite(const UCHAR *clientHello, SIZE_T clientHelloLength, TlsCipherSuite expected)
{
...
bool ExtensionPayloadEquals(const UCHAR *clientHello, SIZE_T clientHelloLength, USHORT leftType, USHORT rightType)
{
    ...
    return FindExtension(clientHello, clientHelloLength, leftType, &left, &leftLength) &&
           ...
           leftLength == rightLength && memcmp(left, right, leftLength) == 0;
}

bool ExtensionPayloadEqualsBytes(const UCHAR *clientHello, SIZE_T clientHelloLength, USHORT extensionType,
                                 const UCHAR *expected, SIZE_T expectedLength)
{
    const UCHAR *extension = nullptr;
    SIZE_T extensionLength = 0;
    return expected != nullptr &&
           FindExtension(clientHello, clientHelloLength, extensionType, &extension, &extensionLength) &&
           extensionLength == expectedLength && memcmp(extension, expected, expectedLength) == 0;
}

bool ClientHelloOffersSignatureScheme(const UCHAR *clientHello, SIZE_T clientHelloLength, TlsSignatureScheme expected)
{
    const UCHAR *extension = nullptr;
    SIZE_T extensionLength = 0;
...
bool Tls13ClientHelloSupportedVersionsAreTls13Only(const UCHAR *clientHello, SIZE_T clientHelloLength)
{
    const UCHAR *extension = nullptr;
    SIZE_T extensionLength = 0;
...
bool KeyShareIsRawX25519(const UCHAR *clientHello, SIZE_T clientHelloLength)
{
    ...
    return group == static_cast<USHORT>(TlsNamedGroup::X25519) && keyLength == 32 && vectorLength == 4 + keyLength &&
           extension[6] != 4;
}
...
void TestDefaultTls13ClientHelloOffersX25519First()
{
    ...
    Expect(FirstSupportedGroupIs(message, written, TlsNamedGroup::X25519),
           "default supported_groups starts with X25519");
    Expect(ClientHelloOffersNamedGroup(message, written, TlsNamedGroup::X448), "default supported_groups offers X448");
    Expect(ClientHelloOffersNamedGroup(message, written, TlsNamedGroup::Ffdhe2048),
           "default supported_groups offers FFDHE2048");
    Expect(ClientHelloOffersNamedGroup(message, written, TlsNamedGroup::Ffdhe8192),
           "default supported_groups offers FFDHE8192");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsAes128GcmSha256),
           "TLS 1.3 ClientHello offers AES_128_GCM_SHA256");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsAes256GcmSha384),
           "TLS 1.3 ClientHello offers AES_256_GCM_SHA384");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsChaCha20Poly1305Sha256),
           "TLS 1.3 ClientHello offers CHACHA20_POLY1305_SHA256");
    Expect(!ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsAes128CcmSha256),
           "TLS 1.3 default ClientHello does not offer AES_128_CCM_SHA256");
    Expect(!ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsAes128Ccm8Sha256),
           "TLS 1.3 default ClientHello does not offer AES_128_CCM_8_SHA256");
    Expect(ExtensionPayloadEquals(message, written, ExtensionSignatureAlgorithms, ExtensionSignatureAlgorithmsCert),
           "TLS 1.3 ClientHello sends signature_algorithms_cert matching signature_algorithms");
    Expect(Tls13ClientHelloSupportedVersionsAreTls13Only(message, written),
           "TLS 1.3 ClientHello supported_versions only offers 0x0304");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::RsaPssRsaeSha512),
           "TLS 1.3 ClientHello offers RSA-PSS-RSAE SHA512");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::RsaPssPssSha512),
           "TLS 1.3 ClientHello offers RSA-PSS-PSS SHA512");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::EcdsaSecp521r1Sha512),
           "TLS 1.3 ClientHello offers ECDSA P-521 SHA512");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::Ed25519),
           "TLS 1.3 ClientHello offers Ed25519");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::Ed448),
...
void TestTls13CustomExtensions()
{
    TlsContext context;
    NTSTATUS status = context.InitializeClient13();
    ExpectStatus(status, STATUS_SUCCESS, "TLS 1.3 context initializes for custom extension");

    const UCHAR transportParameters[] = {1, 2, 3, 4};
    const Tls13ExtensionView customExtension = {ExtensionQuicTransportParameters, transportParameters,
                                                sizeof(transportParameters)};
    Tls13ClientHelloOptions options = {};
    options.CustomExtensions = &customExtension;
    options.CustomExtensionCount = 1;

    UCHAR message[2048] = {};
    SIZE_T written = 0;
    status = TlsHandshake13::EncodeClientHello(context, options, message, sizeof(message), &written);
    ExpectStatus(status, STATUS_SUCCESS, "TLS 1.3 ClientHello encodes a custom extension");
    const UCHAR *encoded = nullptr;
    SIZE_T encodedLength = 0;
    Expect(FindExtension(message, written, ExtensionQuicTransportParameters, &encoded, &encodedLength) &&
               encodedLength == sizeof(transportParameters) &&
               memcmp(encoded, transportParameters, sizeof(transportParameters)) == 0,
           "custom extension payload is preserved");

    const UCHAR extensionBlock[] = {0, 57, 0, 4, 1, 2, 3, 4};
    Tls13ExtensionView view = {};
    bool found = false;
    status = TlsHandshake13::FindExtensionView(extensionBlock, sizeof(extensionBlock), ExtensionQuicTransportParameters,
                                               view, &found);
    ExpectStatus(status, STATUS_SUCCESS, "generic extension view parses");
    Expect(found && view.Type == ExtensionQuicTransportParameters && view.Length == 4 && view.Data[3] == 4,
           "generic extension view returns the requested payload");

    TlsContext duplicateContext;
    status = duplicateContext.InitializeClient13();
    ExpectStatus(status, STATUS_SUCCESS, "duplicate extension context initializes");
    const Tls13ExtensionView duplicates[] = {customExtension, customExtension};
    options.CustomExtensions = duplicates;
    options.CustomExtensionCount = 2;
    status = TlsHandshake13::EncodeClientHello(duplicateContext, options, message, sizeof(message), &written);
    ExpectStatus(status, STATUS_INVALID_PARAMETER, "duplicate custom extension types are rejected");

    TlsContext reservedContext;
    status = reservedContext.InitializeClient13();
    ExpectStatus(status, STATUS_SUCCESS, "reserved extension context initializes");
    const Tls13ExtensionView reserved = {ExtensionKeyShare, transportParameters, sizeof(transportParameters)};
    options.CustomExtensions = &reserved;
    options.CustomExtensionCount = 1;
    status = TlsHandshake13::EncodeClientHello(reservedContext, options, message, sizeof(message), &written);
    ExpectStatus(status, STATUS_INVALID_PARAMETER, "custom extensions cannot replace TLS-owned extensions");
}
...
void TestTls13HelloRetryRequestCanSelectP256AfterX25519Offer()
{
    ...
    TlsNamedGroup groups[] = {TlsNamedGroup::X25519, TlsNamedGroup::Secp256r1, TlsNamedGroup::Secp384r1};

    TlsCipherSuite cipherSuites[] = {TlsCipherSuite::TlsAes128GcmSha256};
...
void TestTls13HelloRetryRequestCanSelectFfdheAfterX25519Offer()
{
    ...
    TlsNamedGroup groups[] = {TlsNamedGroup::X25519, TlsNamedGroup::Ffdhe2048, TlsNamedGroup::Ffdhe3072};
...
void TestTls13EncodeClientCertificateVerifyAndEndOfEarlyData()
{
    ...
    const UCHAR certificateList[] = {0, 0, 1, 0xaa, 0, 0};
    UCHAR message[64] = {};
    SIZE_T written = 0;

    NTSTATUS status = TlsHandshake13::EncodeCertificate(requestContext, sizeof(requestContext), certificateList,
                                                        sizeof(certificateList), message, sizeof(message), &written);
    ExpectStatus(status, STATUS_SUCCESS, "TLS 1.3 client Certificate with credentials encodes");
    Expect(written == 4 + 1 + sizeof(requestContext) + 3 + sizeof(certificateList),
           "TLS 1.3 client Certificate length matches");
    Expect(message[0] == static_cast<UCHAR>(TlsHandshakeType::Certificate),
           "TLS 1.3 credential Certificate type is written");
    ...
    Expect(message[6] == 0 && message[7] == 0 && message[8] == sizeof(certificateList),
           "TLS 1.3 credential Certificate list length is written");

    const UCHAR signature[] = {0x11, 0x22, 0x33};
    status = TlsHandshake13::EncodeCertificateVerify(wknet::tls::TlsSignatureScheme::RsaPssRsaeSha256, signature,
                                                     sizeof(signature), message, sizeof(message), &written);
    ...
    Expect(message[0] == static_cast<UCHAR>(TlsHandshakeType::CertificateVerify),
           "TLS 1.3 CertificateVerify type is written");
    ...
    Expect(memcmp(message + 8, signature, sizeof(signature)) == 0,
           "TLS 1.3 CertificateVerify signature bytes are copied");
    ...
    Expect(message[0] == static_cast<UCHAR>(TlsHandshakeType::EndOfEarlyData),
           "TLS 1.3 EndOfEarlyData type is written");
...
void TestTls12CipherSuiteMetadata()
{
    ...
    if (ecdheRsa != nullptr)
    {
        ...
        Expect(ecdheRsa->Tls12KeyExchange == wknet::tls::Tls12KeyExchangeKind::EcdheRsa,
               "ECDHE_RSA AES-GCM declares ECDHE_RSA key exchange");
        Expect(ecdheRsa->Authentication == wknet::tls::TlsAuthenticationKind::Rsa,
               "ECDHE_RSA AES-GCM declares RSA authentication");
        Expect(ecdheRsa->BulkCipher == wknet::tls::TlsBulkCipherKind::AesGcm,
               "ECDHE_RSA AES-GCM declares AES-GCM bulk cipher");
        Expect(ecdheRsa->RecordMac == wknet::tls::TlsRecordMacKind::Aead,
               "ECDHE_RSA AES-GCM declares AEAD record protection");
        Expect(ecdheRsa->PrfHash == wknet::tls::TlsPrfHashKind::Sha256, "ECDHE_RSA AES-GCM declares SHA-256 PRF");
        Expect(HasRequiredExtension(*ecdheRsa, wknet::tls::TlsCipherSuiteExtensionExtendedMasterSecret),
               "TLS 1.2 ECDHE_RSA AES-GCM requires extended_master_secret");
        Expect(HasRequiredExtension(*ecdheRsa, wknet::tls::TlsCipherSuiteExtensionSecureRenegotiation),
               "TLS 1.2 ECDHE_RSA AES-GCM requires secure renegotiation");
        Expect(HasRequiredExtension(*ecdheRsa, wknet::tls::TlsCipherSuiteExtensionSupportedGroups),
    ...
    {
        Expect(ecdheEcdsa->Tls12KeyExchange == wknet::tls::Tls12KeyExchangeKind::EcdheEcdsa,
               "ECDHE_ECDSA ChaCha20-Poly1305 declares ECDHE_ECDSA key exchange");
        Expect(ecdheEcdsa->Authentication == wknet::tls::TlsAuthenticationKind::Ecdsa,
               "ECDHE_ECDSA ChaCha20-Poly1305 declares ECDSA authentication");
        Expect(ecdheEcdsa->BulkCipher == wknet::tls::TlsBulkCipherKind::ChaCha20Poly1305,
    ...
    if (dheRsa != nullptr)
    {
        Expect(dheRsa->Tls12KeyExchange == wknet::tls::Tls12KeyExchangeKind::DheRsa,
               "DHE_RSA AES-256-GCM declares DHE_RSA key exchange");
        Expect(dheRsa->PrfHash == wknet::tls::TlsPrfHashKind::Sha384, "DHE_RSA AES-256-GCM declares SHA-384 PRF");
        Expect(TlsHandshake12::PrfHashForCipherSuite(TlsCipherSuite::TlsDheRsaWithAes256GcmSha384) ==
    ...
    if (rsaCbc != nullptr)
    {
        Expect(rsaCbc->Tls12KeyExchange == wknet::tls::Tls12KeyExchangeKind::Rsa,
               "RSA AES-CBC declares RSA key exchange");
        Expect(rsaCbc->Authentication == wknet::tls::TlsAuthenticationKind::Rsa,
               "RSA AES-CBC declares RSA authentication");
        Expect(rsaCbc->BulkCipher == wknet::tls::TlsBulkCipherKind::AesCbc, "RSA AES-CBC declares AES-CBC bulk cipher");
        Expect(rsaCbc->RecordMac == wknet::tls::TlsRecordMacKind::HmacSha256,
               "RSA AES-CBC declares HMAC-SHA256 record MAC");
        Expect(HasRequiredExtension(*rsaCbc, wknet::tls::TlsCipherSuiteExtensionEncryptThenMac),
...
{
    TlsPolicy modern = {};
    Expect(wknet::tls::TlsPolicyAllowsCipherSuite(modern, TlsCipherSuite::TlsEcdheRsaWithAes128GcmSha256),
           "modern policy allows TLS 1.2 ECDHE_RSA AES-GCM");
    Expect(wknet::tls::TlsPolicyAllowsCipherSuite(modern, TlsCipherSuite::TlsEcdheEcdsaWithAes128GcmSha256),
           "modern policy allows TLS 1.2 ECDHE_ECDSA AES-GCM");
    Expect(wknet::tls::TlsPolicyAllowsCipherSuite(modern, TlsCipherSuite::TlsDheRsaWithAes128GcmSha256),
           "modern policy allows TLS 1.2 DHE_RSA AES-GCM");
    Expect(wknet::tls::TlsPolicyAllowsCipherSuite(modern, TlsCipherSuite::TlsDheRsaWithChaCha20Poly1305Sha256),
           "modern policy allows TLS 1.2 DHE_RSA ChaCha20-Poly1305");
    Expect(!wknet::tls::TlsPolicyAllowsCipherSuite(modern, TlsCipherSuite::TlsRsaWithAes128GcmSha256),
           "modern policy rejects TLS 1.2 RSA key exchange");
    Expect(!wknet::tls::TlsPolicyAllowsCipherSuite(modern, TlsCipherSuite::TlsEcdheRsaWithAes128CbcSha256),
           "modern policy rejects TLS 1.2 AES-CBC");
    Expect(!wknet::tls::TlsPolicyAllowsSignatureScheme(modern, TlsSignatureScheme::RsaPkcs1Sha1),
           "modern policy rejects TLS 1.2 rsa_pkcs1_sha1");
    Expect(!wknet::tls::TlsPolicyAllowsSignatureScheme(modern, TlsSignatureScheme::EcdsaSha1),
           "modern policy rejects TLS 1.2 ecdsa_sha1");

    modern.EnableTls12Sha1Signatures = true;
    ExpectStatus(wknet::tls::TlsValidatePolicy(modern), STATUS_INVALID_PARAMETER,
    ...
    Expect(!wknet::tls::TlsPolicyAllowsCipherSuite(compatibility, TlsCipherSuite::TlsRsaWithAes128GcmSha256),
           "compatibility policy still requires explicit RSA key exchange opt-in");
    Expect(!wknet::tls::TlsPolicyAllowsCipherSuite(compatibility, TlsCipherSuite::TlsEcdheRsaWithAes128CbcSha256),
           "compatibility policy still requires explicit CBC opt-in");
    Expect(!wknet::tls::TlsPolicyAllowsSignatureScheme(compatibility, TlsSignatureScheme::RsaPkcs1Sha1),
           "compatibility policy keeps rsa_pkcs1_sha1 disabled without SHA1 opt-in");
    Expect(!wknet::tls::TlsPolicyAllowsSignatureScheme(compatibility, TlsSignatureScheme::EcdsaSha1),
    ...
    Expect(!wknet::tls::TlsPolicyAllowsCipherSuite(compatibility, TlsCipherSuite::TlsRsaWithAes128CbcSha256),
...
void TestTls12ClientHelloEncodesExplicitFullMatrix()
{
    ...
    const TlsCipherSuite suites[] = {
        TlsCipherSuite::TlsEcdheRsaWithAes128GcmSha256, TlsCipherSuite::TlsEcdheEcdsaWithAes128GcmSha256,
        TlsCipherSuite::TlsDheRsaWithAes128GcmSha256,   TlsCipherSuite::TlsRsaWithAes128GcmSha256,
        TlsCipherSuite::TlsEcdheRsaWithAes128CbcSha256, TlsCipherSuite::TlsEcdheRsaWithChaCha20Poly1305Sha256};
    ...
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsEcdheRsaWithAes128GcmSha256),
           "TLS 1.2 ClientHello offers ECDHE_RSA AES-GCM");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsEcdheEcdsaWithAes128GcmSha256),
           "TLS 1.2 ClientHello offers ECDHE_ECDSA AES-GCM");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsDheRsaWithAes128GcmSha256),
           "TLS 1.2 ClientHello offers DHE_RSA AES-GCM");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsRsaWithAes128GcmSha256),
           "TLS 1.2 ClientHello offers RSA AES-GCM");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsEcdheRsaWithAes128CbcSha256),
           "TLS 1.2 ClientHello offers AES-CBC");
    Expect(ClientHelloOffersCipherSuite(message, written, TlsCipherSuite::TlsEcdheRsaWithChaCha20Poly1305Sha256),
           "TLS 1.2 ClientHello offers ChaCha20-Poly1305");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::RsaPssRsaeSha512),
           "TLS 1.2 ClientHello offers RSA-PSS-RSAE SHA512");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::RsaPkcs1Sha512),
           "TLS 1.2 ClientHello offers RSA-PKCS1 SHA512");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::EcdsaSecp521r1Sha512),
           "TLS 1.2 ClientHello offers ECDSA P-521 SHA512");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::Ed25519),
           "TLS 1.2 ClientHello offers Ed25519");
    Expect(ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::Ed448),
           "TLS 1.2 ClientHello offers Ed448");

    const UCHAR ecPointFormats[] = {0x01, 0x00};
    Expect(
        ExtensionPayloadEqualsBytes(message, written, ExtensionEcPointFormats, ecPointFormats, sizeof(ecPointFormats)),
...
void TestTls12ClientHelloSignatureSha1Matrix()
{
    ...
    Expect(!ClientHelloOffersSignatureScheme(message, written, TlsSignatureScheme::EcdsaSha1),
           "modern TLS 1.2 ClientHello does not offer ecdsa_sha1");

    const TlsSignatureScheme compatibilitySchemes[] = {TlsSignatureScheme::RsaPssRsaeSha256,
                                                       TlsSignatureScheme::EcdsaSecp256r1Sha256,
                                                       TlsSignatureScheme::RsaPkcs1Sha1, TlsSignatureScheme::EcdsaSha1};
    ...
    written = 0;
    status = TlsHandshake12::EncodeClientHello(compatibilityContext, options, message, sizeof(message), &written);
