...
A：支持显式 opt-in。`wss` 连接设置 `ConnectConfig.AllowWebSocketOverHttp2=true` 后会 offer `h2,http/1.1`，协商到 h2 且对端启用 `SETTINGS_ENABLE_CONNECT_PROTOCOL` 时走 RFC 8441 extended CONNECT；默认仍保持 HTTP/1.1 Upgrade，`ws://` 不隐式走 h2c。permessage-deflate 等扩展不在目标内。
