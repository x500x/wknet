...
| 路径 | 内容 |
|------|------|
| `src/wknettest/samples/HighLevelApiSamples.cpp` | Session、同步/异步 HTTP、Body、TLS、HTTP/2、WebSocket |
...
公共头：`#include <wknet/Wknet.h>`。WebSocket 也可 `#include <wknet/websocket/WebSocket.h>`。
...
| 流式下载 | `SendOptions.OnHeader` / `OnBody` | `SendOptionsCreate` + `SendEx` |
