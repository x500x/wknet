...
- **请求 trailer**：仅在 `Chunked` 请求体路径可用；公共 API 为 `BodyAddTrailer` / `RequestAddTrailer`，禁止字段和 CRLF 注入会被拒绝。
...
- `BodyCreateStream` / `RequestSetBodySource` 使用调用方读取回调按块提供请求体；已知长度发送 `Content-Length`，未知长度配合 `RequestBodyMode::Chunked` 由库生成 chunked framing。
- `BodyAddTrailer` / `RequestAddTrailer` 只在 chunked 请求体结束后发送 trailer。
...
- `SessionConfig.Proxy` 对 HTTPS 使用 CONNECT；对 `http://` 使用 absolute-form request target，不建立 CONNECT 隧道。
