...
| Path | Content |
|------|---------|
| `src/wknettest/samples/HighLevelApiSamples.cpp` | Session, sync/async HTTP, body shapes, TLS, HTTP/2, WebSocket, SSE |
...
| `tests/sse_parser_tests.cpp` / `sse_client_tests.cpp` | SSE parser and client fake-transport regression |

Public header: `#include <wknet/Wknet.h>`. WebSocket may also use `#include <wknet/websocket/WebSocket.h>`; SSE may use `#include <wknet/sse/Sse.h>`.
...
| Streaming download | `SendOptions.OnHeader` / `OnBody` (may fire multiple times) | `SendOptionsCreate` + `SendEx` |
...
| SSE event stream | `HighLevelApiSamples` SSE section, [SSE](sse.md) | `sse::Connect` / `Receive` / `Close` |
...
- SSE: `Event` fields are valid until the next `Receive` / `Close`; do not call `Receive` concurrently on one client
